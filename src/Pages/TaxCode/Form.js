...
function Form(props) {
    ...
    const onSubmit = (data, event) => {
        ...
        if (formState.formType == "New" || formState.formType == "Clone") {
            ...
            CreateData(globalContext, props.data.modelLink, formdata).then(res => {
                if (res.data) {
                    if (res.data.message == "Duplicate Tax Code.") {
                        ToastNotify("error", res.data.message)
                        ControlOverlay(false)
                    }
                    else {
                        ToastNotify("success", "Tax code created successfully.")
                        navigate("/setting/general-settings/tax-code/update/id=" + res.data.data, { state: { formType: "Update", id: res.data.data } })
                    }
                }
            ...
            UpdateData(formState.id, globalContext, props.data.modelLink, formdata).then(res => {
                if (res.data.data) {
                    ToastNotify("success", "Tax code updated successfully.")
                    navigate("/setting/general-settings/tax-code/update/id=" + res.data.data, { state: { formType: "Update", id: res.data.data } })
    ...
    $('.validCheckbox').unbind('change').bind('change', function () {
        if ($(this).prop("checked")) {
            setValue("TaxCode[Valid]", "1")
        } else {
            setValue("TaxCode[Valid]", "0")
        }
    ...
    return (
        <form>
             {formState ? formState.formType == "Clone" || formState.formType == "New" ? <CreateButton handleSubmitData={handleSubmitForm} title='TaxCode' data={props} /> : <UpdateButton handleSubmitData={handleSubmitForm} title="TaxCode" model="tax-code" selectedId="TaxCodeUUIDs" id={formState.id} data={props} /> : <CreateButton handleSubmitData={handleSubmitForm} title='TaxCode' data={props} />}
            <div className="col-md-12">
                <div className="card card-primary">
                    <div className="card-header">Tax Code Form</div>
                    <div className="card-body">

                        <div className="row">
                            <div className="col-xs-12 col-md-4">
                                <div className="form-group">
                                    <label className={`control-label ${errors.TaxCode ? errors.TaxCode.TaxCode? "has-error-label" : "" :""}`}>Port Code</label>
                                    <input defaultValue='' {...register("TaxCode[TaxCode]", { required: "Tax Code cannot be blank." })} className={`form-control ${errors.TaxCode ? errors.TaxCode.TaxCode? "has-error" : "" :""}`} />
                                    <p>{errors.TaxCode ? errors.TaxCode.TaxCode && <span style={{ color:"#A94442" }}>{errors.TaxCode.TaxCode.message}</span> : ""}</p>
                                </div>
                            </div>
                            ...
                            <div className="col-xs-12 col-md-4">
                                <div className="form-group">
                                    ...
                                    <input defaultValue='' {...register("TaxCode[TaxRate]")} className={`form-control`} />
                                </div>
                            </div>
                            ...
                            <div className="col-xs-12 col-md-4">
                                <div className="form-group">
                                    ...
                                    <input defaultValue='' {...register("TaxCode[AccountCode]")} className={`form-control`} />
                                </div>
                            </div>
                            ...
                            <div className="col-xs-12 col-md-12">
                                <div className="form-group">
                                    ...
                                    <textarea defaultValue='' {...register("TaxCode[Description]")} className={`form-control`} />
                                </div>
                            </div>
                            ...
                            <div className="col-xs-12 col-md-3 mt-2">
                                ...
                                <input type="checkbox" className="validCheckbox" id="validCheckbox" defaultChecked />
                                    <input type="text" className="form-control d-none" defaultValue='1' {...register('TaxCode[Valid]')} />
