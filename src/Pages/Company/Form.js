				...
				if (res) {
					...
					if (flag != "") {
						if (flag.includes(used) == true) {
							...
							if (
								$(".PrevCompanyName").val() !==
									getValues("Company[CompanyName]").trim() &&
								$(".PrevROC").val() !== getValues("Company[ROC]").trim()
							) {
					...
					(res) => {
						if (res.data) {
							if (res.data.message == "Company Name has already been taken.") {
								...
								navigate("/company/company/update/id=" + res.data.data, {
				...
				UpdateData(
					...
					if (res.data.data) {
						if (type) {
							...
							navigate(
								"/company/company/update/id=" + res.data.data + "/type=" + type,
								{state: {formType: "Update", id: res.data.data}}
							);
						} else {
							ToastNotify("success", "Company updated successfully.");
		...
		useEffect(() => {
			...
			GetAllDropDown(
				...
				$.each(res.CreditTerm, function (key, value) {
					ArrayCreditTerm.push({
						value: value.CreditTermUUID,
						label: value.CreditTerm,
					});
				});

				$.each(res.CompanyType, function (key, value) {
					ArrayCompanyType.push({
						value: value.CompanyTypeUUID,
						label: value.CompanyType,
					});
				});

				$.each(res.BusinessNature, function (key, value) {
					ArrayBusinessNature.push({
						value: value.BusinessNatureUUID,
						label: value.BusinessNature,
					});
				});

				$.each(res.CustomerType, function (key, value) {
					ArrayCustomerType.push({
						value: value.CustomerTypeUUID,
						label: value.CustomerType,
					});
				});

				$.each(res.SupplierType, function (key, value) {
					ArraySupplierType.push({
						value: value.SupplierTypeUUID,
						label: value.SupplierType,
					});
				});
						...
						(res) => {
							...
							if (state.formType !== "Clone") {
								if (res.data.data.AccessPort) {
									setValue(
										"Company[AccessPort][]",
										res.data.data.AccessPort.split(",")
									);
								} else {
						...
						<div className='card-body'>
							<div className='row'>
								<div className='col-xs-12 col-md-6'>
									<div className='form-group'>
										...
										<input
											defaultValue=''
											{...register("Company[PrevCompanyName]")}
											className={`form-control PrevCompanyName d-none`}
										/>
										<p>
											{errors.Company
												? errors.Company.CompanyName && (
								...
								<div className='col-xs-12 col-md-2'>
									<div className='form-group'>
										...
										<input
											defaultValue=''
											{...register("Company[ROC]", {
												required: "ROC cannot be blank.",
											})}
											className={`form-control ${
												errors.Company
													? errors.Company.ROC
										...
										<input
											defaultValue=''
											{...register("Company[PrevROC]")}
											className={`form-control PrevROC d-none`}
										/>
										<p>
											{errors.Company
												? errors.Company.ROC && (
														<span style={{color: "#A94442"}}>
															{errors.Company.ROC.message}
														</span>
												  )
												: ""}
										</p>
									...
									<div className='form-group'>
											...
											render={({field: {onChange, value}}) => (
												<Select
													...
													{...register("Company[CreditTerm]", {
														required: "Credit Term cannot be blank.",
													})}
													{...register("Company[CreditTerm]")}
													value={
														value
															? creditTerm.find((c) => c.value === value)
															: null
													}
													onChange={(val) => {
													...
													options={creditTerm}
													className={`form-control CreditTerm ${
														errors.Company
															? errors.Company.CreditTerm
																? "has-error-select"
																: ""
										...
										<p>
											{errors.Company
												? errors.Company.CreditTerm && (
														<span style={{color: "#A94442"}}>
															{errors.Company.CreditTerm.message}
														</span>
												  )
												: ""}
										</p>
										...
										<Controller
											...
											render={({field: {onChange, value}}) => (
												<Select
													...
													value={
														value
															? salesPerson.find((c) => c.value === value)
															: null
													}
													onChange={(val) =>
														val == null ? onChange(null) : onChange(val.value)
							...
							<div className='card'>
								<Branch
									...
									BusinessNatureOptions={businessNature}
									CustomerTypeOptions={customerType}
									SupplierTypeOptions={supplierType}
									GenderOptions={Gender}
									TitleOptions={Title}
									PortCodeOptions={portCode}
									CompanyBranchData={passCompanyBranchData}
									Type={type}
								/>
