...
function QuickFormContainer(props) {
    ...
    function handleChangeChargesCode(val, index) {
        ...
        if (val) {
            GetChargesById(val.value, globalContext).then(res => {
                ...
                setValue(`SalesDebitNoteHasItem[${index}][UOM]`, arrayUOM[0])
                setValue(`SalesDebitNoteHasItem[${index}][ChargesName]`, res.data.ChargesName)
                setValue(`SalesDebitNoteHasItem[${index}][UnitPrice]`, res.data.ReferencePrice)
    ...
    function handleChangeTaxCode(val, index) {
        ...
        if (val) {
            GetTaxCodeById(val.value, globalContext).then(res => {
                ...
                setValue(`SalesDebitNoteHasItem[${index}][TaxRate]`, res.data.TaxRate ? parseFloat(res.data.TaxRate).toFixed(2) : "")
    ...
    function handleCalculate(index) {
        var qty = getValues(`SalesDebitNoteHasItem[${index}][Qty]`)
        var unitPrice = getValues(`SalesDebitNoteHasItem[${index}][UnitPrice]`)
        var currency = getValues(`SalesDebitNoteHasItem[${index}][Currency]`)
        var currencyRate = getValues(`SalesDebitNoteHasItem[${index}][CurrencyExchangeRate]`)
        ...
        setValue(`SalesDebitNoteHasItem[${index}][TotalAmount]`, TotalAmount)
        setValue(`SalesDebitNoteHasItem[${index}][Amount]`, TotalAmount)
        ...
        var taxRate = getValues(`SalesDebitNoteHasItem[${index}][TaxRate]`)
        if (taxRate) {
            if (taxRate.includes("%")) {
            ...
            setValue(`SalesDebitNoteHasItem[${index}][TaxAmount]`, TotalTax)
            // setTotalDiscount(unitTotalDiscount)
        }
        else {
            SubTotalLocal = (parseFloat(TotalAmount)).toFixed(2)
            SubTotal = (parseFloat(TotalAmount)).toFixed(2)
            setValue(`SalesDebitNoteHasItem[${index}][TaxAmount]`, "0.00")
        }
        ...
        if (currency !== $("input[name='SalesDebitNote[Currency]']").val()) {    
            if (currencyRate !== "") {
                var foreigncurrency = 1;
                var subtotallocalexchange = (parseFloat(SubTotalLocal) * parseFloat(currencyRate)).toFixed(2);
               
                setValue(`SalesDebitNoteHasItem[${index}][SubTotalLocal]`, subtotallocalexchange)
            }
        }
        else {
            setValue(`SalesDebitNoteHasItem[${index}][SubTotalLocal]`, SubTotalLocal)
          
        }
        setValue(`SalesDebitNoteHasItem[${index}][TotalTax]`, TotalTax)
        setValue(`SalesDebitNoteHasItem[${index}][SubTotal]`, SubTotal)
        ...
        $.each($(".ParentTaxAmount"), function (key, value) {
            var tempTotalTax = getValues(`SalesDebitNoteHasItem[${key}][TaxAmount]`)
            var tempCurrency= getValues(`SalesDebitNoteHasItem[${key}][Currency]`)
            var tempCurrencyRate=getValues(`SalesDebitNoteHasItem[${index}][CurrencyExchangeRate]`)


            if(tempCurrency!== $("input[name='SalesDebitNote[Currency]']").val()){
                if (tempTotalTax) {
        ...
        $.each($(".ParentSubTotalLocal"), function (key, value) {
            var tempSubTotal = getValues(`SalesDebitNoteHasItem[${key}][SubTotalLocal]`)
            if (tempSubTotal) {
    ...
    if (getCookie('salesdebitnotecolumn')) {
        var getCookieArray = getCookie('salesdebitnotecolumn');
        ...
        if (AllChargesArray.length > 0) {
            GetBCChargesDescrption(AllChargesArray, globalContext).then(res => {
                ...
                $(".container-items").each(function (key, value) {
                    ...
                    setValue(`SalesDebitNoteHasItem[${key}][ChargesDescription]`, chargesdiscription)
    ...
    useEffect(() => {
        ...
        if (props.transferPartialData) {
            ...
            var arrayDynamic = []
            remove()
            var newContainerData = props.transferPartialData.SalesDebitNoteHasItem
            var AllChargesArray = [];

            if(props.barge){

                $.each(props.transferPartialData.SalesDebitNote, function (key2, value2) {
                  if(key2!=="DocDate" && key2!=="SalesPerson"){
                    props.setValue('SalesDebitNote[' + key2 + ']', value2);
                    props.setValue('DynamicModel[' + key2 + ']', value2);
                  }
                   
                })
                formContext.setStateHandle([{ label: props.transferPartialData.SalesDebitNote.VoyageName + "(" + props.transferPartialData.SalesDebitNote.VesselCode + ")", value: props.transferPartialData.SalesDebitNote.VoyageNum }], "VoyageNum")
                formContext.setStateHandle([{ label: props.transferPartialData.SalesDebitNote.VoyageName + "(" + props.transferPartialData.SalesDebitNote.VesselCode + ")", value: props.transferPartialData.SalesDebitNote.VoyageNum }], "QuickFormVoyageNum")
                

                if (props.transferPartialData.SalesDebitNote.POLLocationCode) {
                    formContext.setStateHandle([{ value: props.transferPartialData.SalesDebitNote.POLLocationCode, label: props.transferPartialData.SalesDebitNote.pOLLocationCode.PortName }], "OptionPOLTerminal")
                }
                if (props.transferPartialData.SalesDebitNote.PODLocationCode) {
                    formContext.setStateHandle([{ value: props.transferPartialData.SalesDebitNote.PODLocationCode, label: props.transferPartialData.SalesDebitNote.pODLocationCode.PortName }], "OptionPODTerminal")
                }
    
                if (props.transferPartialData.SalesDebitNote.ShipOperator) {
                    $("#CompanyROC-Voyage-DetailForm").val(props.transferPartialData.SalesDebitNote.shipOperator.ROC)
                    $("#BranchCode-Voyage-DetailForm").val(props.transferPartialData.SalesDebitNote.shipOperatorBranchCode.BranchCode)
                
                
                }
    
                if (props.transferPartialData.SalesDebitNote.POLHandlingOfficeCode) {
                 
                    formContext.setStateHandle([{ value: props.transferPartialData.SalesDebitNote.pOLHandlingOfficeCode.company.CompanyUUID, label: props.transferPartialData.SalesDebitNote.pOLHandlingOfficeCode.company.CompanyName }], "OptionPOLAgentCompany")
                    props.setValue("SalesDebitNote[POLAgentName]", props.transferPartialData.SalesDebitNote.pOLHandlingOfficeCode.company.CompanyUUID)
                    props.setValue("SalesDebitNote[POLAgentROC]", props.transferPartialData.SalesDebitNote.pOLHandlingOfficeCode.company.ROC)
                    formContext.setStateHandle([{ value: props.transferPartialData.SalesDebitNote.POLHandlingOfficeCode, label: props.transferPartialData.SalesDebitNote.pOLHandlingOfficeCode.BranchCode }], "OptionPOLAgentCompanyBranch")
                }
    
                if (props.transferPartialData.SalesDebitNote.PODHandlingOfficeCode) {
                    formContext.setStateHandle([{ value: props.transferPartialData.SalesDebitNote.pODHandlingOfficeCode.company.CompanyUUID, label: props.transferPartialData.SalesDebitNote.pODHandlingOfficeCode.company.CompanyName }], "OptionPODAgentCompany")
                    props.setValue("SalesDebitNote[PODAgentName]", props.transferPartialData.SalesDebitNote.pODHandlingOfficeCode.company.CompanyUUID)
                    props.setValue("SalesDebitNote[PODAgentROC]", props.transferPartialData.SalesDebitNote.pODHandlingOfficeCode.company.ROC)
                    formContext.setStateHandle([{ value: props.transferPartialData.SalesDebitNote.PODHandlingOfficeCode, label: props.transferPartialData.SalesDebitNote.pODHandlingOfficeCode.BranchCode }], "OptionPODAgentCompanyBranch")
                }

                if (props.transferPartialData.SalesDebitNote.FinalDestinationHandler) {
                    props.setValue("SalesDebitNote[FinalDestinationHandler]", { CompanyUUID: props.transferPartialData.SalesDebitNote.finalDestinationHandler.CompanyUUID, CompanyName: props.transferPartialData.SalesDebitNote.finalDestinationHandler.CompanyName })
                }
    ...
    const { register, handleSubmit, setValue, trigger, getValues, reset, control, watch, formState: { errors } } = useForm({
        mode: "onChange",
        defaultValues: {
            SalesDebitNoteHasItem: [{ "Name": `${formName}HasItem`,"SeqNum":$(".container-items").length+1, "Qty": 1, "ContainerItem": ContainerColumn, ContainerTypeOptions: props.containerType }]
        }
    });
    const {
        fields,
    ...
    useEffect(() => {
        if (removeState) {
            handleCalculate(0)
            $.each(fields, function (key, value) {
                setValue(`SalesDebitNoteHasItem[${key}][SeqNum]`, key)
            })
            update(fields)
        }
                                                                        ...
                                                                        if (index2 == 0) {
                                                                            return (
                                                                                ...
                                                                                <td className={item2.class}>
                                                                                    <input defaultValue='' {...register(`${formName}HasItem` + '[' + index + ']' + '[SalesDebitNoteHasItemUUID]')} className={`form-control d-none`} />
                                                                                    <input defaultValue='' {...register(`${formName}HasItem` + '[' + index + ']' + '[SalesDebitNoteHasContainerType]')} className={`form-control d-none`} />
                                                                                ...
                                                                                <td className={item2.class}>
                                                                                    {item2.requiredField ?
                                                                                        <input defaultValue={item2.defaultValue ? item2.defaultValue : ""} readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasItem` + '[' + index + ']' + '[' + item2.name + ']', { required: "required" })}
                                                                                            className={`form-control ${item2.fieldClass ? item2.fieldClass : ""} ${errors[`${formName}HasItem`] ? errors[`${formName}HasItem`][`${index}`] ? errors[`${formName}HasItem`][`${index}`][`${item2.name}`] ? "has-error" : "" : "" : ""}`} onBlur={(val) => { val ? item2.onBlur(val, index) : item2.onBlur(null, index) }} />
                                                                                        :
                                                                                        <input defaultValue={item2.defaultValue ? item2.defaultValue : ""} readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasItem` + '[' + index + ']' + '[' + item2.name + ']')} className={`form-control ${item2.fieldClass ? item2.fieldClass : ""}`} onBlur={(val) => { val ? item2.onBlur(val, index) : item2.onBlur(null, index) }} />
                                                                                    }
