...
import DetailFormInstructionMultiplePOD from '../../Components/CommonElement/DetailFormInstructionMultiplePOD'
import FormContext from '../../Components/CommonElement/FormContext';
import $ from "jquery";

function InstructionMultitplePOD(props) {
  const formContext = useContext(FormContext)
    ...
    useEffect(() => {
        if (props.instructionData) {
            $("#quotation-quickform-dndcombinedday").val(props.instructionData.DNDCombinedDay)
            ...
            if (props.instructionData.POLHandlingOfficeCode) {
                ...
                props.setValue("Quotation[POLAgentName]", props.instructionData.pOLHandlingOfficeCodeCompany.CompanyUUID)
                props.setValue("Quotation[POLAgentROC]", props.instructionData.pOLHandlingOfficeCodeCompany.ROC)
            ...
            if (props.instructionData.PODHandlingOfficeCode) {
                ...
                props.setValue("Quotation[PODAgentName]", props.instructionData.pODHandlingOfficeCodeCompany.CompanyUUID)
                props.setValue("Quotation[PODAgentROC]", props.instructionData.pODHandlingOfficeCodeCompany.ROC)
            ...
            if (props.instructionData.FinalDestinationHandler) {
                props.setValue("Quotation[FinalDestinationHandler]", { CompanyUUID: props.instructionData.finalDestinationHandler.CompanyUUID, CompanyName: props.instructionData.finalDestinationHandler.CompanyName })
            }
            if(props.instructionData.InsistTranshipment == '1'){
                $(`input[name='Quotation[InsistTranshipment]']`).parent().find("input:checkbox").prop("checked",true)
            }

            if(props.instructionData.AutoBilling == '1'){
                $(`input[name='Quotation[AutoBilling]']`).parent().find("input:checkbox").prop("checked",true)
            }
            if(props.instructionData.ApplyDND == '0'){
                $(`#quotation-applydnd`).parent().find("input:checkbox").prop("checked",false)
                ...
                if(props.instructionData.DNDCombined == '0'){
                    $(`#quotation-dndcombined`).parent().find("input:checkbox").prop("checked",false)
                    $(`#quotation-quickform-dndcombined`).parent().find("input:checkbox").prop("checked",false)
                    $(".DNDCombineDay").addClass("d-none")
    ...
    return (
        <div className={`DetailFormDetails Instructions`}>
            <div className="quotation-instruction-form">
                <DetailFormInstructionMultiplePOD register={props.register} control={props.control} errors={props.errors} InstructionItem={InstructionItem} setValue={props.setValue} port={props.port} getValues={props.getValues}/>
            </div>
        </div>
    ) 
}

export default InstructionMultitplePOD