...
import { GetUpdateData, CreateData, UpdateData, ToastNotify,createCookie, GetRemainingInvoice, sortArray, getCROTransferFromBooking,GetCNDNTransferFromSalesInvoice, ControlOverlay, GetBookingReservationContainerQty, initHoverSelectDropownTitle, GetAllDropDown, GetUserDetails, getAreaById, getPortDetails, getPortDetailsById, getVoyageByIdSpecial } from '../../Components/Helper.js'
...
function Form(props) {
    ...
    const onSubmit = (data, event) => {
        ...
        if (formState.formType == "New" || formState.formType == "Clone" || formState.formType == "TransferFromINV" ) {
            ...
            CreateData(globalContext, props.data.modelLink, formdata).then(res => {
                if (res.data) {
                    if (res.data.message == "Credit Note has already been taken.") {
                        ToastNotify("error", res.data.message)
                        ControlOverlay(false)
                    }
                    else {
                        ToastNotify("success", "Credit Note created successfully.")
                        if(props.data.modelLink=="credit-note-barge"){
            ...
            UpdateData(formState.id, globalContext, props.data.modelLink, formdata).then(res => {
                if (res.data.data) {
                    ToastNotify("success", "Credit Note updated successfully.")
                    if(props.data.modelLink=="credit-note-barge"){
                        navigate(`/sales/standard/${props.data.modelLink}/update/id=` + res.data.data, { state: { formType: "Update", id: res.data.data } })
                    }else{
                        navigate(`/sales/container/${props.data.modelLink}/update/id=` + res.data.data, { state: { formType: "Update", id: res.data.data } })
    ...
    useEffect(() => {
        ...
        $("#salescreditnotebillto-branchcode").off("change").on("change", function () {
            console.log(props)
        ...
        $(document).on("change", ".columnChooserColumn", function (event) {
            var creditNoteCookies = []
            $(this).parent().parent().find(".columnChooserColumn:checked").each(function () {
    
                creditNoteCookies.push($(this).parent().index())
    
            });
            var json_str = JSON.stringify(creditNoteCookies);
            createCookie('creditnotecolumn', json_str, 3650);
            if (fields.length > 0) {
    ...
    function TransferFromINV(id,array){
        ...
        GetCNDNTransferFromSalesInvoice(id, props.data.modelLink,globalContext,array).then(res => {
            ...
            if (res.data.SalesCreditNote) {
                setDocumentData(res.data.SalesCreditNote)
                setInstructionData(res.data.SalesCreditNote)

                $.each(res.data.SalesCreditNote, function (key2, value2) {
                    ...
                    if(key2=="SalesPerson"){
                        ...
                        setValue('SalesCreditNote[' + key2 + ']', value2);
            ...
            if (res.data.SalesCreditNoteHasItem) {
                setContainerQuickFormData(res.data.SalesCreditNoteHasItem)
            }

            if(res.data.SalesCreditNote.salesInvoice){
                  
                setInvoiceList([{label:res.data.SalesCreditNote.salesInvoice.DocNum,value:res.data.SalesCreditNote.salesInvoice.SalesInvoiceUUID}])
                setValue("InvoiceNo",res.data.SalesCreditNote.salesInvoice.SalesInvoiceUUID)
            }

          

            if (res.data.SalesCreditNoteMore) {

                setMoreData(res.data.SalesCreditNoteMore)
            }

            if (res.data.SalesCreditNote) {
                ArrayAttention= res.data.SalesCreditNote
            }
    

            if (res.data.SalesCreditNote.VerificationStatus == "Pending") {
                $(".VerificationStatusField").text("Draft")
                $(".VerificationStatusField").removeClass("text-danger")
                setVerificationStatus("Pending")
            } else if (res.data.SalesCreditNote.VerificationStatus == "Rejected") {
    ...
    function GetUpdateCLoneData(id) {
        GetUpdateData(id, globalContext, props.data.modelLink).then(res => {
            ...
            if (res.data.data.SalesCreditNote) {
                setDocumentData(res.data.data.SalesCreditNote)
                setInstructionData(res.data.data.SalesCreditNote)

                $.each(res.data.data.SalesCreditNote, function (key2, value2) {
                    if(key2=="CurrencyExchangeRate"){
                        value2=value2?parseFloat(value2).toFixed(2):""
                    }else if(key2=="TotalTax"){
                        value2=value2?parseFloat(value2).toFixed(2):""
            ...
            if (res.data.data.SalesCreditNoteHasItem) {
                ...
                setContainerQuickFormData(res.data.data.SalesCreditNoteHasItem)
            ...
            if (res.data.data.SalesCreditNote) {
                ...
                setMoreData(res.data.data.SalesCreditNoteMore)
            }

            if (res.data.data.SalesCreditNote) {
                ArrayAttention= res.data.data.SalesCreditNote
            }
    

            if (res.data.data.SalesCreditNote.VerificationStatus == "Pending") {
                $(".VerificationStatusField").text("Draft")
                $(".VerificationStatusField").removeClass("text-danger")
                setVerificationStatus("Pending")
            } else if (res.data.data.SalesCreditNote.VerificationStatus == "Rejected") {
                $(".VerificationStatusField").text("Rejected")
                $(".VerificationStatusField").addClass("text-danger")
                setVerificationStatus("Rejected")
            }
    ...
    useEffect(() => {
        ...
        GetUserDetails(globalContext).then(res => {
            ...
            setValue("SalesCreditNote[Agent]", res[0]["Company"]["CompanyUUID"])
            setValue("SalesCreditNote[AgentBranch]", res[0]["Branch"]["CompanyBranchUUID"])
            setValue("SalesCreditNote[AgentCompanyName]", res[0]["Company"]["CompanyName"])
            setValue("SalesCreditNote[AgentBranchName]", res[0]["Branch"]["BranchName"])
    ...
    return (
        <form>
           {formState ? formState.formType == "Clone" || formState.formType == "New" || formState.formType =="TransferFromINV" ? <CreateButton handleSubmitData={handleSubmitForm}  title='SalesCreditNote' data={props} RemaningBR={bookingReservation} /> : <UpdateButton handleSubmitData={handleSubmitForm}  title="SalesCreditNote" model="credit-note" selectedId="SalesCreditNoteUUIDs" id={formState.id} data={props} voyageDelay={voyageDelay} position="bottom" /> : <CreateButton handleSubmitData={handleSubmitForm}  title='SalesCreditNote' data={props} />}
            <div className="">
                <div className="box">
                            ...
                            <FormContext.Provider value={{
                                ...
                                <div id="toggleForm" className="card card-success flex-item-right mb-0 pr-0" style={{ display: "none" }}>
                                    ...
                                    <div className="card-body cardMaxHeight">
                                        ...
                                        {props.data.modelLink=="credit-note-barge"?<Document formType={formState.formType} documentData={documentData} user={user} currency={currency} port={port} register={register} setValue={setValue} control={control} errors={errors} />:""}
                                        {props.data.modelLink=="credit-note-barge"?<Instruction instructionData={instructionData} register={register} setValue={setValue} control={control} getValues={getValues} errors={errors} port={port}  bargeCode={bargeCode}/>:""}
                                        <More moreData={moreData} state={state} register={register} setValue={setValue} control={control} errors={errors} />
                                      
                                    </div>
                                </div>
                                {/* end: detail form  */}
                                <ShareInitialize formName="SalesCreditNote" formNameLowerCase="salescreditnote" setValue={setValue} trigger={trigger} getValues={getValues} globalContext={globalContext} />
                    ...
                    <div className="shortcut-buttons sticky">
                        <ul className="sticky nav nav-pills flex-column">
                            ...
                            {props.data.modelLink=="credit-note-barge"?<button type="button" style={{ border: 0, backgroundColor: 'white' }} className="navigate-icon icon text-secondary" id="Document" data-toggle="tooltip" data-placement="left" data-target="Document" data-original-title="Document"><i className="fa fa-file" /></button>:""}
                            ...
                            {props.data.modelLink=="credit-note-barge"?<button type="button" style={{ border: 0, backgroundColor: 'white' }} className="navigate-icon icon text-secondary" id="Instructions" data-toggle="tooltip" data-placement="left" data-target="Instructions" data-original-title="Instructions"><i className="fab fa-artstation" /></button>:""}
