...
import { GetUpdateData, CreateData, UpdateData, ToastNotify, FindRemainingBR, getCROTransferFromBooking, ControlOverlay, GetBookingReservationContainerQty, initHoverSelectDropownTitle, GetAllDropDown, GetUserDetails, getAreaById, getPortDetails, getPortDetailsById, getVoyageByIdSpecial } from '../../Components/Helper.js'
...
function Form(props) {
    ...
    const onSubmit = (data, event) => {
        ...
        if (formState.formType == "New" || formState.formType == "Clone" || formState.formType == "TransferFromBooking") {
            ...
            CreateData(globalContext, props.data.modelLink, formdata).then(res => {
                if (res.data) {
                    if (res.data.message == "Container Release Order has already been taken.") {
    ...
    useEffect(() => {
        ...
        FindRemainingBR(globalContext).then(res => {
            ...
            setBookingReservation(ArrayBooking)
    ...
    useEffect(() => {
        ...
        //console.log(getValues("ContainerReleaseOrder[BookingReservation]"))
        return () => {
        }
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            {formState ? formState.formType == "Clone" || formState.formType == "New" ? <CreateButton title='ContainerReleaseOrder' data={props} RemaningBR={bookingReservation} /> : <UpdateButton title="ContainerReleaseOrder" model="container-release-order" selectedId="ContainerReleaseOrderUUIDs" id={formState.id} data={props} /> : <CreateButton title='ContainerReleaseOrder' data={props} RemaningBR={bookingReservation} />}
            <div className="">
                <div className="box">
