...
import { createCookie, getCookie, GetChargesByAreaContainer,sortArray, GetChargesById, GetTaxCodeById, initHoverSelectDropownTitle } from '../../Components/Helper.js'
            ...
            $.each(newData, function (key, value) {
                if (key == props.containerIndex) {
                    ...
                    $.each(newValue.ContainerCode, function (key2, value2) {
                        ...
                        var resultSeal =  props.checkPrevContainerInnerData.filter(item => item.ContainerCode ==newValue2.ContainerUUID).map(item => item.SealValue);
                        if( resultSeal.length>0){
                            newValue2.innerContainerItem[1].defaultValue = resultSeal.join("")
                        }
        ...
        $.each(newData, function (key, value) {
            if (key == props.containerIndex) {
                ...
                $.each(newValue.ContainerCode, function (key2, value2) {
                    ...
                    newValue2.innerContainerItem[0].options = arrayOption
                    // newValue2.innerContainerItem[1].defaultValue = newValue.SealNum

                    var resultSeal = savedSeal.filter(item => item.ContainerCode ==newValue2.ContainerUUID).map(item => item.SealValue);
                    if( resultSeal.length>0){
                        newValue2.innerContainerItem[1].defaultValue = resultSeal.join("")
                        newValue2.SealNum = resultSeal.join("")
                    }
                                                ...
                                                if (item2.inputType == "input") {
                                                    return (
                                                        <td className={item2.class}>
                                                            <input defaultValue={item2.defaultValue?item2.defaultValue:""}  {...register("ContainerReleaseOrderHasContainer" + '[' + props.containerIndex + '][ContainerCode]' + "[" + index + "]" + "[" + item2.name + "]")} className={`form-control ${item2.fieldClass ? item2.fieldClass : ""}`} />
                                                        </td>
                                                    )
                                                }
