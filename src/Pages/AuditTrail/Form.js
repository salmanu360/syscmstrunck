    ...
    function operateFormatter(value, row, index) {
			...
			$.each(paramsTable.actions, function (key, values) {
				if (values["type"] == "Preview") {
					...
					if (
						ModuleTrim == "ContainerRelease" ||
						ModuleTrim == "ContainerGateIn" ||
						ModuleTrim == "ContainerLoaded" ||
						ModuleTrim == "ContainerDischarged" ||
						ModuleTrim == "ContainerGateOut" ||
						ModuleTrim == "ContainerReceive"
					) {
						actionButtons +=
							'<a href="javascript:void(0)"  className="PreviewAuditTrail"><i className="' +
							values["icon"] +
							'" style="color:grey;"></i></a> ';
						//   actionButtons += '<button type="buttton" className="btn btn-xs btn-primary" ><i className="'+values['icon']+'"></i></button> ';
					} else {
						actionButtons +=
							'<a href="javascript:void(0)"  className="PreviewAuditTrail"><i className="' +
							values["icon"] +
		...
		function columnSetup(columns) {
			...
			if (columns) {
				...
				if (
					getCookie(`${props.data.modelLink}-table.bs.table.reorderColumns`)
				) {
					var getCookieArray = getCookie(
						`${props.data.modelLink}-table.bs.table.reorderColumns`
					);
					getCookieArray = JSON.parse(getCookieArray);

					const newArray = Object.keys(getCookieArray)
					...
					const uniqueArray = Concatarray.filter(
						(item, index, self) =>
		...
		function handleGenerate() {
			...
			var defaultHide = [
				// default field to hide in bootstrap table
			];
			var columns = [
				// data from controller (actionGetCompanyType) field and title to be included
				{
			...
			var GetGridviewData = function (params) {
				...
				$.ajax({
					...
					url:
						...
						"audit-trail/get-audit-trails-latest",
					data: {
						param: param,
						User: User,
						Create: Create,
			...
			initTable({
				...
				actions: [
					// action column for bootstrap table
					{
						type: "Preview",
						url: "",
						icon: "fa fa-file-pdf",
		...
		function initTable(args) {
				...
				.bootstrapTable({
					...
					onLoadSuccess: function () {
						...
						if (
							window
								.$(`#${props.data.modelLink}-table`)
								.bootstrapTable("getCookies")["columns"] == null
						) {
							$.each(args.hideColumns, function (key, value) {
								window
									.$(`#${props.data.modelLink}-table`)
									.bootstrapTable("hideColumn", value);
							});
						}
					},
				});
			...
			window
				.$(`#${props.data.modelLink}-table`)
				.on("reorder-column.bs.table", function (e, args) {
					var newLatestColumn = [];
					if (
						getCookie(`${props.data.modelLink}-table.bs.table.reorderColumns`)
					) {
			...
			window
				...
				.on("click-cell.bs.table", function (field, value, row, $element) {
					if (value == "operate") {
						...
						if (
							module == "ContainerRelease" ||
							module == "ContainerGateIn" ||
							module == "ContainerLoaded" ||
							module == "ContainerDischarged" ||
							module == "ContainerGateOut" ||
							module == "ContainerReceive"
						) {
							return false;
						}
						...
						$.ajax({
							...
							url:
								globalContext.globalHost +
								globalContext.globalPathLink +
								"audit-trail/get-audit-trail-detail?id=" +
								id +
								"&module=" +
								moduleDetail +
								"&action=" +
							...
							success: function (data) {
								...
								$.each(data, function (key1, value1) {
									if (count == 1) {
										$("#myTab").append(
											'<li className="nav-item" role="presentation"><a className="nav-link active" id="' +
												key1 +
										...
										$("#myTabContent").append(
											'<div className="tab-pane fade show active" id="' +
												...
												key1 +
												'-tab"><table style="table-layout:fixed;width: 100%;" className="table table-bordered"><thead><tr><th>Field</th><th>Old Value</th><th>New Value</th></tr></thead><tbody id="' +
												key1 +
												'Body"></tbody></table></div>'
										);
									} else {
										$("#myTab").append(
											'<li className="nav-item" role="presentation"><a className="nav-link" id="' +
												key1 +
												'-tab" data-toggle="tab" href="#' +
												key1 +
												'" role="tab" aria-controls="' +
									...
									$.each(value1, function (key, value) {
										if (value.length != undefined) {
											$("#" + key1 + "Body").append(
												'<tr style="font-weight:bold;" className="lvl1"><td colspan="3">' +
													key1 +
													" " +
													(key + 1) +
													"</td></td>"
											...
											$.each(value, function (key3, value3) {
												if (value3.field != undefined) {
													...
													if (checkValue.indexOf("UUID") == -1) {
														if (
															checkValue.indexOf("user") != -1 ||
															checkValue.indexOf("action") != -1 ||
															checkValue.indexOf("date") != -1 ||
															checkValue.indexOf(module) != -1 ||
															checkValue.indexOf("_Version") != -1
														) {
														} else {
													...
													$.each(value3, function (key6, value6) {
														//each charges
														$("#" + key1 + "Body").append(
															'<tr style="font-weight:bold;" className="lvl2"><td colspan="3" style="padding-left:30px;">' +
																key3 +
																" " +
																(key6 + 1) +
																"</td></td>"
															...
															$.each(value6, function (key7, value7) {
																if (value7.field != undefined) {
																	if (value7.new_value == null) {
																		...
																		if (
																			value7.new_value_reference !== undefined
																		) {
																			var newValue = value7.new_value_reference;
																		} else {
																			var newValue = value7.new_value;
																	...
																	if (value7.old_value == null) {
																		...
																		if (
																			value7.old_value_reference !== undefined
																		) {
																			var oldValue = value7.old_value_reference;
																		} else {
																			var oldValue = value7.old_value;
																	...
																	$("#" + key1 + "Body").append(
																		'<tr style="font-weight:bold;" className="lvl2"><td colspan="3" style="padding-left:45px;">' +
																			key7 +
																			" " +
																			(key6 + 1) +
																			"</td></td>"
