...
import { GetUpdateData, CreateData, UpdateData, ToastNotify,FindBLRemainingBR,ControlOverlay,sortArray, initHoverSelectDropownTitle, GetAllDropDown, GetUserDetails } from '../../Components/Helper.js'
...
function Form(props) {
    ...
    const onSubmit = (data, event) => {
        ...
        if(props.data.modelLink=="delivery-order-barge"){
            formdata.forEach((value, key) => {
                const newKey = key.replace('DeliveryOrderHasContainer', 'DeliveryOrderCharges');
        ...
        if (formState.formType == "New" || formState.formType == "Clone") {
            ...
            CreateData(globalContext, props.data.modelLink, props.data.modelLink=="delivery-order-barge"?bargeFormData:formdata).then(res => {
                if (res.data) {
                    if (res.data.message == "Bill Of Lading has already been taken.") {
                        ...
                        if(props.data.modelLink=="delivery-order-barge"){
                            ...
                            navigate("/operation/standard/delivery-order-barge/update/id=" + res.data.data, { state: { formType: "Update", id: res.data.data } })
                        }else{
                            ToastNotify("success", "Bill Of Lading created successfully.")
                            navigate("/operation/container/delivery-order/update/id=" + res.data.data, { state: { formType: "Update", id: res.data.data } })
                        }
            ...
            UpdateData(formState.id, globalContext, props.data.modelLink, props.data.modelLink=="delivery-order-barge"?bargeFormData:formdata).then(res => {
                if (res.data.data) {
                    if(props.data.modelLink=="delivery-order-barge"){
                        ToastNotify("success", "Delivery Order updated successfully.")
                        navigate("/operation/standard/delivery-order-barge/update/id=" + res.data.data, { state: { formType: "Update", id: res.data.data } })
                    }else{
                        ToastNotify("success", "Delivery Order updated successfully.")
                        navigate("/operation/container/delivery-order/update/id=" + res.data.data, { state: { formType: "Update", id: res.data.data } })       
    ...
    useEffect(() => {
        if (docDate) {
            ...
            $("#deliveryorder-voyagenum").val("").trigger("change")
    ...
    useEffect(() => {
        ...
        GetUserDetails(globalContext).then(res => {
            setValue("DeliveryOrderAgent[ROC]", res[0]["Company"].ROC)
    ...
    function GetUpdateCLoneData(id) {
        GetUpdateData(id, globalContext, props.data.modelLink).then(res => {
            ...
            if (res.data.data.DeliveryOrder) {
                setDocumentData(res.data.data.DeliveryOrder)
                setInstructionData(res.data.data.DeliveryOrder)
                setShippingInstructionQuickFormData(res.data.data.DeliveryOrder)
                $.each(res.data.data.DeliveryOrder, function (key2, value2) {

                    setValue('DeliveryOrder[' + key2 + ']', value2);


                })
                if(res.data.data.DeliveryOrder.SplitParent==null){
            ...
            if (res.data.data.DeliveryOrder.VerificationStatus == "Pending") {
                $(".VerificationStatusField").text("Draft")
                $(".VerificationStatusField").removeClass("text-danger")
            } else if (res.data.data.DeliveryOrder.VerificationStatus == "Rejected") {
            ...
            if (res.data.data.DeliveryOrderHasContainer) {
                if(props.data.modelLink=="delivery-order-barge"){
                    setContainerQuickFormData(res.data.data.DeliveryOrderCharges)
                }else{
                    setContainerQuickFormData(res.data.data.DeliveryOrderHasContainer)
                }
               
            }

            if (res.data.data.DeliveryOrderHauler) {

                setHaulerData(res.data.data.DeliveryOrderHauler)
                ArrayMiddleCard["Hauler"] = res.data.data.DeliveryOrderHauler
            }

            if (res.data.data.DeliveryOrderMore) {

                setMoreData(res.data.data.DeliveryOrderMore)
            }

            if (res.data.data.DeliveryOrderAgent) {
                ArrayAttention["DeliveryOrderAgent"] = res.data.data.DeliveryOrderAgent
            }
            if (res.data.data.DeliveryOrderBillTo) {
                ArrayAttention["DeliveryOrderBillTo"] = res.data.data.DeliveryOrderBillTo
            }
            if (res.data.data.DeliveryOrderShipper) {
                ArrayAttention["DeliveryOrderShipper"] = res.data.data.DeliveryOrderShipper
            }

            if (res.data.data.DeliveryOrderConsignee) {
                ArrayAttention["DeliveryOrderConsignee"] = res.data.data.DeliveryOrderConsignee
            }

            if (res.data.data.DeliveryOrderPartyExt) {
                ArrayAttention["DeliveryOrderPartyExt"] = res.data.data.DeliveryOrderPartyExt
            }
            ...
            setBookingReservationDoc(res.data.data.DeliveryOrder.BookingReservation)
    ...
    useEffect(() => {
        ...
        remove()
        FindBLRemainingBR(globalContext).then(res => {
            var ArrayBooking = [];
        ...
        $(".BookingLink").off("click").on('click', function () {
            var id = getValues("DeliveryOrder[BookingReservation]")
         if(props.data.modelLink=="delivery-order-barge"){
        ...
        $(".BillOfLadingLink").off("click").on('click', function () {
            var id = getValues("DeliveryOrder[BillOfLading]")
            if(props.data.modelLink=="delivery-order-barge"){
                window.open('../../../standard/bill-of-lading-barge/update/id=' + id, '_blank')
            }else{
                window.open('../../../container/bill-of-lading/update/id=' + id, '_blank')
            }   
        ...
        $(".QuotationLink").off("click").on('click', function () {
            var id = getValues("DeliveryOrder[Quotation]")
            if(props.data.modelLink=="delivery-order-barge"){
    ...
    return (
        <form>
            {formState ? formState.formType == "Clone" || formState.formType == "New"   ? <CreateButton handleSubmitData={handleSubmitForm} barge={props.data.modelLink=="delivery-order-barge"?true:false} title='DeliveryOrder' data={props} RemaningBR={bookingReservation} formType={formState.formType} /> : <UpdateButton handleSubmitData={handleSubmitForm}  barge={props.data.modelLink=="delivery-order-barge"?true:false} title="DeliveryOrder" model="delivery-order" selectedId="DeliveryOrderUUIDs" id={formState.id} data={props} /> : <CreateButton handleSubmitData={handleSubmitForm} barge={props.data.modelLink=="delivery-order-barge"?true:false} title='DeliveryOrder' data={props} RemaningBR={bookingReservation} formType={formState.formType} />}
            <div className="">
                <div className="box">
                    <div className="left-form">
                        <div className="flex-container">
                            <FormContext.Provider value={{
                                ...
                                optionPODAgentCompanyBranch,bargeCode, pOLReqETA, pODReqETA, portTerm, freightTerm, defaultFinalDestinationCompany, setDefaultFinalDestinationCompany, VoyageNum, quickFormVoyageNum
                            }}>
                                ...
                                <div id="toggleForm" className="card card-success flex-item-right mb-0 pr-0" style={{ display: "none" }}>
                                    ...
                                    <div className="card-body cardMaxHeight">
                                        ...
                                        <Instruction barge={props.data.modelLink=="delivery-order-barge"?true:false} instructionData={instructionData} register={register} setValue={setValue} control={control} getValues={getValues} errors={errors} port={port} bargeCode={bargeCode} />
                                ...
                                {/* end: detail form  */}
                                <ShareInitialize formName="DeliveryOrder" formNameLowerCase="deliveryorder" setValue={setValue} getValues={getValues} globalContext={globalContext} />
                                <VoyageModal formName="DeliveryOrder" formNameLowerCase="deliveryorder" register={register} setValue={setValue} control={control} getValues={getValues} errors={errors} port={port} />
                                <ContainerModal formName="DeliveryOrder" formNameLowerCase="deliveryorder" register={register} setValue={setValue} control={control} getValues={getValues} errors={errors} />
                                <CurrencyModal formName="DeliveryOrder" formNameLowerCase="deliveryorder" register={register} setValue={setValue} control={control} getValues={getValues} errors={errors} />
