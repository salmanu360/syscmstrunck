  ...
  useEffect(() => {
			...
			if (props.position == "top") {
				...
				GetVoyageDelay(globalContext, props.model, props.id).then((res) => {
					...
					$.each(res.data, function (key, value) {
						...
						$(".voyage-delay-item").append(
							"<tr><td>" +
								value.VoyageNumber +
								"</td><td>" +
								value.PortCodeName +
								"</td><td className='DelayETA'>" +
								ETA +
								"</td><td className='DelayETD'>" +
								ETD +
								"</td><td className='DelaySCN'>" +
								SCN +
								"</td><td className='DelayClosingDate'>" +
								ClosingDateTime +
								"</td'><td className='PortType d-none'>" +
								value.PortType +
								"</td><td className='TranshipmentSeq d-none'>" +
								value.Transhipment +
								"</td></tr>"
						);
					});
	...
	function handleRemoveFirst() {
		...
		window
			.$("#ButtonVerifyConfirmModal")
			.find(".message")
			.text("Are you sure you want to reject  this data?");
	}
	...
	function handleApproved() {
		if (userRuleSet.includes(`verify-${tempModel}`)) {
			...
			alert(
				"You are not allowed to perform Approved, Please check your Permission."
			);
		}
	}
	...
	function handleCancelApprovedRejectFirst() {
		...
		window
			.$("#ButtonVerifyConfirmModal")
			.find(".message")
			.text("Are you sure you want to cancel approved/reject this data?");
	}
		...
		if (userRuleSet.includes(`verify-${tempModel}`)) {
			...
			Reset(globalContext, props.model, object).then((res) => {
				if (res.Success.length > 0) {
					...
					navigate(
						"/" +
							props.data.data.groupLink +
							props.data.data.modelLink +
							"/index"
					);
			...
			alert(
				"You are not allowed to perform Reset, Please check your Permission."
			);
		}
	}
	...
	function handleSuspend() {
		if (userRuleSet.includes(`verify-${tempModel}`)) {
			...
			alert(
				"You are not allowed to perform Suspend, Please check your Permission."
			);
		}
	}
	...
	function handleThrow() {
		if (userRuleSet.includes(`throw-${tempModel}`)) {
			...
			alert(
				"You are not allowed to perform Throw, Please check your Permission."
			);
		}
	}
	function handleVerify() {
		if (userRuleSet.includes(`verify-${tempModel}`)) {
			...
			alert(
				"You are not allowed to perform Verify, Please check your Permission."
			);
		}
	}
	...
	function handleRemove() {
		if (userRuleSet.includes(`delete-${tempModel}`)) {
			...
			alert(
				"You are not allowed to perform Delete, Please check your Permission."
			);
		}
	}
				...
				Verify(globalContext, props.model, object, type).then((res) => {
					if (res.Failed && res.Failed.length > 0) {
						...
						if (res.Verify[0].SalesInvoice) {
							ToastNotify(
								"success",
								`Successfully approved,Sales Invoice ${res.Verify[0].SalesInvoice} generated`,
								3000
							);
							navigate(
								"/" +
							...
							res.Verify[0].message ==
							"Cannot generate Sales Invoice, empty charges for Freight Prepaid"
						) {
							ToastNotify(
								"warning",
								"Successfully approved,no Sales Invoice generated, empty charges for Freight Prepaid"
							);
							navigate(
								"/" +
									props.data.data.groupLink +
	...
	function handleFinalCancelApproved() {
		...
		if (props.model == "user") {
			...
			cancelApprovedReject(globalContext, props.model, object, type).then(
				(res) => {
					if (res.Pending.length > 0) {
	...
	function handleFinalReject() {
		...
		if (props.model == "user") {
			...
			if (window.$(".rejectMessage").val() !== "") {
				ControlOverlay(true);
				RejectRecord(globalContext, props.model, objectRejectMessage).then(
					(res) => {
			...
			navigate(
					...
					BC,
				{
					state: {
						...
						transferFromModel: "booking-confirmation",
					},
				}
			);
		} else {
		...
		if (type) {
			if (type == "containerReleaseOrderOri") {
				...
				urlLink =
					...
					"/preview?id=" +
					props.id +
					"&ReportType=ShippingOrderDeclaration";
				...
				urlLink =
					...
					globalContext.globalPathLink +
					newModel +
					"/preview-confirmation?id=" +
					props.controlButtonState.BookingConfirmation;
			}
	...
	function handleConfirmVoyageDelay(Type) {
			...
			.each(function (key, value) {
				...
				if ($(value).find(".PortType").text() == "Transhipment") {
					if ($(value).find(".DelayETA").text() !== "") {
						obj2["DischargingDate"] = $(value)
							.find(".DelayETA")
							.text()
							.split("→")[1];
					}
					if ($(value).find(".DelayETD").text() !== "") {
						obj2["LoadingDate"] = $(value)
							.find(".DelayETD")
							.text()
							.split("→")[1];
					}
					var Seq = $(value).find(".TranshipmentSeq").text();
					obj2[`${props.title}HasTranshipmentUUID`] = $(".TranshipmentCardRow")
						.eq(Seq)
						.find(`.${props.title}HasTranshipmentUUID`)
						.val();
				} else {
					if ($(value).find(".DelayETA").text() !== "") {
					...
					if ($(value).find(".DelaySCN").text() !== "") {
						obj[type + "SCNCode"] = $(value)
							.find(".DelaySCN")
							.text()
							.split("→")[1];
					}
					if ($(value).find(".DelayClosingDate").text() !== "") {
						obj["ClosingDateTime"] = $(value)
							.find(".DelayClosingDate")
							.text()
							.split("→")[1];
					}
				}
		...
		objPass["Action"] = Type;
		CheckVoyageDelay(globalContext, props.model, props.id, objPass).then(
			(res) => {
				if (
					res.message == "Check voyage delay successfully" ||
					res.message == "Success"
				) {
					window.$("#VoyageDelayModal").modal("toggle");
					navigate(
						"/" +
							...
							"/update/id=" +
							props.id,
						{state: {id: props.id, formType: "Update"}}
					);
				}
			}
		);
	...
	function handleRealRemove() {
		...
		Remove(globalContext, props.model, object, type).then((res) => {
			if (res.Success.length > 0) {
				...
				navigate(
					"/" + props.data.data.groupLink + props.data.data.modelLink + "/index"
				);
			}
			...
			if (res.Failed.length > 0) {
				...
				ToastNotify(
					"error",
					DocNumArray.toString() + " Failed to Remove.",
					5000
				);
			}
		});
	...
	function handleTransferBR() {
		var tempString;
		props.barge
			? (tempString = "create-booking-reservation-barge")
			: (tempString = "create-booking-reservation");
		if (
			userRuleSet.includes(`transfer-${tempModel}`) &&
			userRuleSet.includes(tempString)
		) {
			var d = new Date();
			...
			if (dateObj1 > dateObj2) {
				...
				if (props.barge) {
					navigate(
						props.data.data.groupLink +
							"booking-reservation-barge/transfer-from-quotation/id=" +
							props.id,
						{
							state: {
								formType: "Transfer",
					...
					navigate(
						props.data.data.groupLink +
							"booking-reservation/transfer-from-quotation/id=" +
							props.id,
						{
							state: {
								formType: "Transfer",
	...
	function handleSplitBL() {
		...
		GetBillOfLadingContainers(globalContext, props.model, props.id).then(
			(res) => {
	...
	function handleSplitBR() {
		...
		GetBookingReservationsContainers(globalContext, props.model, props.id).then(
			(res) => {
	...
	function handleConfirmSplitBL() {
		if (
			userRuleSet.includes(`split-${tempModel}`) &&
			userRuleSet.includes(`create-${tempModel}`)
		) {
			var arraySplit = [];
			var arrayShare = [];
	...
	function handleMergeBL() {
		GetMergeBLList(
			props.id,
			globalContext,
			$("input[name='BillOfLading[POLPortCode]']").val(),
			$("input[name='BillOfLading[PODPortCode]']").val(),
			$("input[name='BillOfLading[VoyageNum]']").val()
		).then((res) => {
			var arrayBLs = [];
	...
	function handleMergeBR() {
		GetMergeBRList(
			props.id,
			globalContext,
			$("input[name='BookingReservation[POLPortCode]']").val(),
			$("input[name='BookingReservation[PODPortCode]']").val(),
			$("input[name='BookingReservation[VoyageNum]']").val(),
			$("input[name='BookingReservation[Quotation]']").val()
		).then((res) => {
			var arrayBRs = [];
			window.$.each(res.data.data, function (key, value) {
	...
	function handleConfirmBR() {
		...
		if (props.controlButtonState.Quotation) {
			CreateBC([props.id], globalContext, props.data.data.modelLink).then(
				(res) => {
					if (res.message) {
					...
					if (res.Confirmed.length >= 1) {
						...
						ToastNotify(
							"error",
							"Failed Create Booking Confirmation.Quotation are Required."
						);
						ControlOverlay(false);
					}
				}
			...
			alert(
				"You are not allow to Confirm this Booking, Please Check Quotation Number for this Booking."
			);
			ControlOverlay(false);
		}
	}
	...
	function confirmMergeBL() {
		if (
			userRuleSet.includes(`merge-${tempModel}`) &&
			userRuleSet.includes(`update-${tempModel}`)
		) {
			...
			navigate(
				"/" +
					...
					"&mergeid=" +
					MergedIDs,
				{state: {bLId: props.id, mergeIDs: MergedIDs, formType: "MergeBL"}}
			);
		} else {
			alert(
	...
	function confirmMergeBR() {
		if (
			...
			CheckingMergeBookingReservation(globalContext, props.id, MergedIDs).then(
				(res) => {
					if (res) {
						if (res.message) {
	...
	function TransferAllBR() {
		...
		if (TransferType == "CRO") {
			...
			if (
				userRuleSet.includes(`transferto-${tempModel}`) &&
				userRuleSet.includes(`create-container-release-order`)
			) {
				if ($(`input[name="DynamicModel[Quotation]"]`).val() != "") {
					if (props.controlButtonState.VerificationStatus == "Approved") {
						...
						alert(
							"You are not allow to transfer to Container Release Order, Please Verify Booking before Transfer."
						);
					}
				} else {
					alert(
						"You are not allow to transfer to Container Release Order, Please Check Quotation Number for this Booking."
					);
				}
			} else {
				alert(
				...
				if (BC == "" || BC == null) {
					...
					$(".SelectContainerCodeField").each(function () {
						if (!$(this).hasClass("ForPartial")) {
							if (
								window.$(this).children().children().last().children().val() ==
								""
							) {
					...
					if (catchEmtpyContainer == true) {
						...
						alert(
							"You are not allow to transfer to Sales Invoice, Please Verify Booking before Transfer."
						);
					} else {
						getRemainingBCbyID(BC, globalContext).then((res) => {
							if (res) {
								...
								$.each($(".insidecharges-item"), function (key, value) {
									...
									data["CustomerType"] = $(this)
										.find(".BillToTypeCharges")
										.children()
										.last()
										.val();
									data["BillTo"] = temp;
								...
								$.each(uniqueData, function (key, value) {
									...
									const radio = $(
										`<input type="radio" className="mr-2" id="radio-${key}" name="billTo" value="` +
											value.BillTo.value +
											'">'
									);
									const label = $(
										`<label className="control-label" for="radio-${key}">` +
											value.CustomerType +
											" - " +
											value.BillTo.label +
											"</label>"
