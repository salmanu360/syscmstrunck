...
export function ShareContainerModelMultiplePOD({ formName, index, fields, getValues, setValue, update, globalContext,portCodeIndex }) {
  ...
  if (formName == "ContainerReleaseOrder") {
    ...
    console.log(portCodeIndex)
  
      console.log(portCodeIndex)
      var ContainerType = getValues(`${formName}POD[${portCodeIndex}][${formName}HasContainerType][${index}][ContainerType]`)
      var OwnershipType = getValues(`${formName}POD[${portCodeIndex}][${formName}HasContainerType][${index}][BoxOwnership]`)
      var select2ContainerCode = getValues(`${formName}POD[${portCodeIndex}][${formName}HasContainerType]` + '[' + index + ']' + '[ContainerCode][]')
  ...
  window.$("#ChooseContainer").click(function () {
    ...
    if (formName == "ContainerReleaseOrder"){
      ...
      Quantity = getValues(`${formName}POD[${portCodeIndex}][${formName}HasContainerType][${index}][Qty]`)
    }
      ...
      if(ContainerUUIDs.length>0){
        getContainers("", filters, globalContext).then(data => {
          ...
          if (formName == "ContainerReleaseOrder") {
            ...
            setValue(`${formName}POD[${portCodeIndex}][${formName}HasContainerType][${index}][ContainerOptions]`, optionContainerList)
            setValue(`${formName}POD[${portCodeIndex}][${formName}HasContainerType][${index}][ContainerCode][]`, ContainerUUIDs)
            update(fields)
          }
        ...
        if (formName == "ContainerReleaseOrder") {
          ...
          setValue(`${formName}POD[${portCodeIndex}][${formName}HasContainerType][${index}][ContainerOptions]`, [])
          setValue(`${formName}POD[${portCodeIndex}][${formName}HasContainerType][${index}][ContainerCode][]`, [])
          update(fields)
        }
