...
function QuickFormShippingInstruction(props) {
  ...
  function potVoyageAllocation(e) {
    ...
    $.each($(".AllocateVoyageButton1"), function (key, value) {
      if (current == value || $(current).parent()[0] == value) {
        ...
        allocatePortCode = $($(".AllocatePortCode")[key])
          .parent()
          .find("input:hidden")
          .val();
        allocateVoyage = $($(".AllocateVoyage")[key])
          .parent()
          .find("input:hidden")
          .val();
        text = $($(".AllocateVoyage")[key])
    ...
    if (portCodeLocation) {
      if ($(portCodeLocation).hasClass("pol_portcode")) {
        ...
        voyageScheduleUUID = $(
          `input[name='${formName}HasTranshipment[${checkIndex}][FromVoyagePOT]']`
        ).val();
      }
    }
    ...
    if (allocateVoyage) {
      ...
      if (allocateVoyage == "") {
        ...
        FindAllocation(filter, globalContext).then((data) => {
          ...
          var usedweight = data.data["UsedWeight"]
            ? data.data["UsedWeight"]
            : 0;
          ...
          var content =
            "Tues: " +
            parseFloat(remainingTues).toFixed(2) +
            " / " +
            parseFloat(tues).toFixed(2) +
            " <br> Weight:" +
            parseFloat(remainingWeight).toFixed(2) +
            " / " +
  ...
  function FindVoyageFromTranshipmentDetails(currentSelector, id, index) {
    if (currentSelector) {
      props.setValue(
        `${formName}HasTranshipment[${index}][FromVoyageNum]`,
    ...
    if (value) {
      ...
      getPortDetails(id, globalContext).then((data) => {
        ...
        if (positionId.includes("pol")) {
          ...
          props.setValue(
            `${formName}[POLHandlingOfficeCode]`,
            DefaultAgentCompanyBranch
          );
          props.setValue(
            `${formName}[POLHandlingOfficeName]`,
            DefaultAgentCompanyBranchName
          );
        } else {
  ...
  function FindPOTVoyage(index) {
    var POTPortCode = [];
    if (index > 0) {
      var POL = props.getValues(
        `${formName}HasTranshipment[${index - 1}][QuickFormPortCode]`
      );
      var POD = props.getValues(
    ...
    if (props.getValues(`${formName}[QuotationType]`) == "Advance Booking") {
      ...
      enddate = $(
        `input[name='${formName}[AdvanceBookingLastValidDate]']`
      ).val();
    } else {
    ...
    if (POL != "" && POD != "") {
      ...
      FindVoyagesWithPolPod(filter, globalContext).then((data) => {
        ...
        props.setValue(
          `${formName}HasTranshipment[${index}][optionFromVoyage]`,
          VoyageArray
        );
        formContext.update(formContext.fields);
  ...
  function onChangePOTPortCode(value, positionId, index) {
    ...
    if (value) {
      props.setValue(
        `${formName}HasTranshipment[${index}][PortCode]`,
        value.value
      );
      ...
      getPortDetails(id, globalContext).then((data) => {
        ...
        // set Option Terminal
        props.setValue(
          `${formName}HasTranshipment[${index}][optionTerminal]`,
          tempOptions
        );
        props.setValue(
          `${formName}HasTranshipment[${index}][optionAgentCompany]`,
          tempOptionsCompany
        );
        props.setValue(
          `${formName}HasTranshipment[${index}][optionAgentBranchCode]`,
          tempOptionsCompanyBranch
        );
        formContext.update(formContext.fields);
        props.setValue(
          `${formName}HasTranshipment[${index}][LocationCode]`,
          DefaultValue
        );
        props.setValue(
          `${formName}HasTranshipment[${index}][LocationName]`,
          DefaultPortName
        );
        props.setValue(
          `${formName}HasTranshipment[${index}][POTHandlingCompanyROC]`,
          DefaultAgentCompanyROC
        );
        props.setValue(
          `${formName}HasTranshipment[${index}][POTHandlingCompanyCode]`,
          DefaultAgentCompany
        );
        props.setValue(
          `${formName}HasTranshipment[${index}][POTHandlingOfficeCode]`,
          DefaultAgentCompanyBranch
        );
        props.setValue(
          `${formName}HasTranshipment[${index}][POTHandlingOfficeName]`,
          DefaultAgentCompanyBranchName
        );
      });
    } else {
  ...
  function onChangePOTVoyageNum(currentSelector, id, index, checkingStatus) {
    ...
    if (currentSelector) {
      ...
      var index = index;
      var closestVesselCode = $(
        `#${formNameLowerCase}hastranshipment-${index}-quickformpotvoyage`
      )
        .parent()
        .parent()
        .next()
        .find(".QuickFormVesselCode");
      var filters = {
        VoyageUUID: $(this).val(),
      };
      ...
      var PodPortcode = props.getValues("DynamicModel[PODPortCode]");
      var PotPortcode = props.getValues(
        `${formName}HasTranshipment[${index}][QuickFormPortCode]`
      );
      var prevPortCode;
      ...
      if (index > 0) {
        startingIndex = index - 1;
        startingVesselName = props.getValues(
          "#" +
            formNameLowerCase +
            "hastranshipment-" +
            startingIndex +
      ...
      getVoyageByIdSpecial(result, globalContext).then((data) => {
        ...
        $.each(data[0]["voyageSchedules"], function (key, value) {
          if (value.PortCode == PolPortcode) {
            if (countPOT == repeatedVoyage) {
              if (index == 0) {
                ...
                $("#" + formNameLowerCase + "-voyage-pol").val(
                  value.VoyageScheduleUUID
                );
                ...
                if (!value.LocationCode) {
                  ...
                  $.each(value.terminalSelection, function (key1, value1) {
                    if (value1.PortDetailsUUID == value.LocationCode) {
                      ...
                      props.setValue(
                        `${formName}[POLLocationName]`,
                        value1.LocationCode
                      );
                      props.setValue(
                        `${formName}[POLAgentROC]`,
                        value1.handlingCompany.AgentCode
                      );
                      props.setValue(
                        `${formName}[POLAgentName]`,
                        value1.handlingCompany.CompanyUUID
                      );
                      props.setValue(
                        `${formName}[POLHandlingOfficeCode]`,
          ...
          if (value.PortCode == PotPortcode) {
            ...
            props.setValue(
              `${formName}HasTranshipment[${index}][FromVoyagePOT]`,
              value.VoyageScheduleUUID
            );
          }
          if (value.PortCode == prevPortCode) {
            //  var ETA = (value.ETA).split(" ");
            props.setValue(
              `${formName}HasTranshipment[${startingIndex}][LoadingDate]`,
              value.ETA
            );
            props.setValue(
              `${formName}HasTranshipment[${startingIndex}][ToVoyagePOT]`,
              value.VoyageScheduleUUID
            );
          }
        ...
        if (index == 0) {
          ...
          $("#" + formNameLowerCase + "-vesselcode").val(
            data[0]["vessel"]["VesselCode"]
          );
          $("#" + formNameLowerCase + "-voyagename").val(StrvoyageNo);
          $.each(data, function (key, value) {
            $("#" + formNameLowerCase + "-vesselname").val(
              value.vessel.VesselName
            );
          });
        }
        ...
        setTimeout(() => {
          $.each(formContext.transhipmentForTransfer, function (key, value) {
            FindVoyageFromTranshipmentDetails(
              value,
              `${formNameLowerCase}hastranshipment-${key}-fromvoyagenum`,
              key
            );
            ...
            props.setValue(
              `${formName}HasTranshipment[${key}][optionFromVoyage]`,
              [value]
            );
            formContext.update(formContext.fields);
            props.setValue(
              `${formName}HasTranshipment[${key}][QuickFormPOTVoyage]`,
              value.value
            );
          });
        }, 100);
                  ...
                  >
                    <div className="col-12 transhipment">
                      <div className="row">
                        ...
                        <div
                          class={`col-md-3 mr-4 transhipmentVoyagedisplay ${
                            formName == "Quotation" &&
                        ...
                        >
                          ...
                          <div class="form-group input-group mb-3">
                            ...
                            >
                              <button
                                class="btn btn-outline-secondary AllocateVoyageButton1"
                                type="button"
                                style={{ height: "31px" }}
                                data-toggle="popover"
                        ...
                        <div
                          class={`col-md-3 transhipmentVesseldisplay ${
                            formName == "Quotation" &&
                            (checkQuotationType === true ? "" : "d-none")
                          }`}
                        >
                          ...
                          <div class="form-group input-group mb-3">
                            <input
                              class="transhipmentQuickform QuickFormVesselCode form-control"
                              {...props.register(
                                `${formName}HasTranshipment[${index}][QuickFormPOTVesselCode]`
                              )}
                            ></input>
              ...
              <div className="col-md-3">
                <div className="form-group field-dynamicmodel-podportcode">
                  <label
                    className={`control-label ${
                      props.errors.DynamicModel
                        ? props.errors.DynamicModel.PODPortCode
                          ? "has-error-label"
                          : ""
                        : ""
                    }`}
                  ...
                  <p>
                    {props.errors.DynamicModel
                      ? props.errors.DynamicModel.PODPortCode && (
                          <span style={{ color: "#A94442" }}>
                            {props.errors.DynamicModel.PODPortCode.message}
                          </span>
                        )
                      : ""}
                  </p>
