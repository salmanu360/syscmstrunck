...
function NestedTableCharges(props) {
    ...
    function openModalCurrencyRate(e) {
		...
		function handleCalculate(index) {
			...
			var qty = getValues(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][Qty]`
			);
			var unitPrice = getValues(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][UnitPrice]`
			);
			...
			var unitDiscount = getValues(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][Discount]`
			);
			...
			setValue(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][Amount]`,
				TotalAmount
			);
			...
			var taxRate = getValues(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][TaxRate]`
			);
			if (taxRate) {
				if (taxRate.includes("%")) {
			...
			var currencyRate = getValues(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][CurrencyExchangeRate]`
			);
			if (currencyRate) {
				SubTotalLocal = (
			...
			setValue(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][TaxAmount]`,
				TotalTax
			);
			setValue(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][SubTotal]`,
				SubTotal
			);
			setValue(
				`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${chargesIndex}][SubTotalLocal]`,
				SubTotalLocal
			);
			...
			$.each($(".ParentDiscount"), function (key, value) {
				var qtyForTotalDisc = getValues(
					`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${key}][Qty]`
				);
				if (!qtyForTotalDisc) {
					qtyForTotalDisc = 0;
				}
				var unitPriceForTotalDisc = getValues(
					`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${key}][UnitPrice]`
				);
				if (!unitPriceForTotalDisc) {
					unitPriceForTotalDisc = 0.0;
				}
				var totalAmountForTotalDisc = getValues(
					`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${key}][Amount]`
				);
			...
			$.each($(".ParentTaxAmount"), function (key, value) {
				var tempTotalTax = getValues(
					`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${key}][TaxAmount]`
				);
				if (tempTotalTax) {
			...
			$.each($(".subtotallocal"), function (key, value) {
				var tempSubTotal = getValues(
					`${props.formName}HasContainerType[${containerIndex}][${props.formName}Charges][${key}][SubTotalLocal]`
				);
				if (tempSubTotal) {
			...
			var chargesColumn = [
				{
					...
					class: "",
					fieldClass:
						"ParentChargesCode ChargesCode Charges_Code readOnlySelect",
					options: [],
				...
				},
				{
					...
					name: "FreightTerm",
					class: "",
					fieldClass: "FreightTerm ParentFreightTerm readOnlySelect",
				...
				{
					columnName: "Charges Code Break Down",
					inputType: "checkbox",
					defaultChecked: false,
					defaultValue: 0,
					name: "ChargesCodeBreakDown",
					class: "d-none",
					fieldClass: "ChargesCodeBreakDownParent",
					onChange: "",
					disabled: true,
				},
				{
					columnName: "Pricing Break Down",
					inputType: "checkbox",
					defaultChecked: false,
					defaultValue: 0,
					name: "PricingBreakDown",
					class: "d-none",
					fieldClass: "PricingBreakDownParent",
					onChange: "",
					disabled: true,
				},
				{
				...
				},
				{
					...
					name: "UOM",
					class: "d-none",
					fieldClass: "ParentUOM readOnlySelect",
					options: [],
				},
				...
				},
				{
					...
					name: "CargoType",
					class: "d-none",
					fieldClass: "CargoType ParentCargoType readOnlySelect",
				...
				},
				{
					...
					name: "TaxCode",
					class: "d-none",
					fieldClass: "TaxCode Tax_Code ParentTaxCode readOnlySelect",
				...
				{
					columnName: "Bill To Type",
					inputType: "single-select",
					defaultChecked: true,
					name: "CustomerType",
					class: "d-none",
					fieldClass: "BillToTypeCharges readOnlySelect",
					options: OptionBIlltoType,
				...
				},
				{
					...
					name: "BillTo",
					class: "d-none",
					fieldClass: "BillToNameCharges readOnlySelect",
					options: [],
					onChange: changeBillTo,
				},

				{
					...
					name: "PortCode",
					class: "d-none",
					fieldClass: "Port_Code readOnlySelect",
					options: props.port,
			...
			if (props.containerChangeIndex == props.containerIndex) {
				if (props.onChangeContainerTypeCharges) {
					if (fields.length > 0) {
						if (
							fields[0].Name ==
							`${props.formName}HasContainerType[${props.containerChangeIndex}][${props.formName}Charges]`
						) {
				...
				if (childLength > 0) {
					for (var index = 0; index < childLength; index++) {
						$(".ChargesTable")
							.eq(props.containerIndex)
							.find(".ParentChargesCode")
		...
		useEffect(() => {
			if (formContext.formState.formType == "New") {
				...
				if (FilledUpdateData != false) {
					var result = props.onChangeContainerTypeChargesVoyage.filter(
						function (item, index) {
							return index == props.containerIndex;
						}
					);
			...
			if (props.barge && props.formName == "Quotation") {
				var voyageUUID =
					$(".transhipment-items").length <= 0
						...
						: $(
								`input[name="${props.formName}HasTranshipment[0][FromVoyageNum]"]`
						  ).val();

				getChargesWithPortCodeAndEmptyContainerType(
					voyageUUID,
					globalContext
				).then((res) => {
				...
				tempCharges[0]["options"] =
					props.combinedChargesOptions[convertedIndex];
				tempCharges[1]["defaultValue"] = $(
					`input[name='${props.formName}HasContainerType[${props.containerIndex}][Qty]']`
				).val();
				append({
		...
		useEffect(() => {
					...
					if (!props.transferPartial) {
						setValue(
							`${props.formName}HasContainerType` +
								...
								props.containerIndex +
								"]" +
								`[${props.formName}Charges][${index}][CustomerType]`,
							"Bill To"
						);
						setTimeout(() => {
							$(
			...
			$(".columnchooserdropdowncharges").on("change", function (event) {
				if (countForBlock == 1) {
					...
					if (props.transferPartial) {
						createCookie(
							`${formNameLowerCase}transferpartialchargescolumn`,
							json_str,
							3650
						);
					} else {
							...
							GetChargesById(value, globalContext).then((data) => {
								...
								if (fields.length > 0) {
									$.each(fields[index]["ChargesItem"], function (key2, value2) {
										if (value2.name == "UOM") {
											setValue(
												`${props.formName}HasContainerType[${props.containerIndex}][${props.formName}Charges][${index}][ChargesItem][${key2}][options]`,
												sortArray(optionUOM)
											);
											update(fields);
										}
								...
								setValue(
									`${props.formName}HasContainerType` +
										...
										props.containerIndex +
										"]" +
										`[${props.formName}Charges][${index}][ChargesName]`,
								...
								setValue(
									`${props.formName}HasContainerType` +
										...
										props.containerIndex +
										"]" +
										`[${props.formName}Charges][${index}][UOM]`,
									defaultUOM
								);
								setValue(
									`${props.formName}HasContainerType` +
										...
										props.containerIndex +
										"]" +
										`[${props.formName}Charges][${index}][FreightTerm]`,
								...
								setValue(
									`${props.formName}HasContainerType` +
										...
										props.containerIndex +
										"]" +
										`[${props.formName}Charges][${index}][ChargesDescription]`,
								...
								setValue(
									`${props.formName}HasContainerType` +
										...
										props.containerIndex +
										"]" +
										`[${props.formName}Charges][${index}][TaxCode]`,
									data.data.TaxCode
								);
								setValue(
									`${props.formName}HasContainerType` +
										...
										props.containerIndex +
										"]" +
										`[${props.formName}Charges][${index}][TaxRate]`,
									data.data.TaxRate
								);
								setValue(
									`${props.formName}HasContainerType` +
										...
										props.containerIndex +
										"]" +
										`[${props.formName}Charges][${index}][Currency]`,
								...
								setValue(
									`${props.formName}HasContainerType` +
										...
										props.containerIndex +
										"]" +
										`[${props.formName}Charges][${index}][UnitPrice]`,
										...
										if (
											...
											setValue(
												`${props.formName}HasContainerType` +
													...
													props.containerIndex +
													"]" +
													`[${props.formName}Charges][${index}][CurrencyExchangeRate]`,
					...
					for (var index = 0; index < childLenght; index++) {
						setValue(
							`${props.formName}HasContainerType` +
								...
								props.containerIndex +
								"]" +
								`[${props.formName}Charges][${index}][Qty]`,
		...
		useEffect(() => {
			...
			$.each(props.loadTariffState, function (key, value) {
				if (props.containerIndex == key) {
					...
					$.each(
						value.tariffHasContainerTypeCharges[0].SelectCharges,
						function (key4, value4) {
							var obj;
							if (value4.portCode) {
								obj = {
									value: value4.ChargesUUID,
									label:
										value4.ChargesCode + "(" + value4.portCode.PortCode + ")",
								};
							} else {
					...
					$.each(value.tariffHasContainerTypeCharges, function (key2, value2) {
						...
						value2[
							"Name"
						] = `${props.formName}HasContainerType[${key}][${props.formName}Charges]`;
						value2["Qty"] = $(
							`input[name='${props.formName}HasContainerType[${key}][Qty]']`
						).val();
						...
						if (foundData == false) {
							value2.ChargesOption.push({
								value: value2.ChargesCode,
								label: value2.chargesCode.ChargesCode,
							});
						}
						append(value2);
						...
						if (value2.tariffHasContainerTypeCharges.length > 0) {
							setTimeout(() => {
								$(
									`input[name='${props.formName}HasContainerType[${props.containerIndex}][${props.formName}Charges][${key2}][UnitPrice]']`
								).prop("readonly", true);
								$(
									`input[name='${props.formName}HasContainerType[${props.containerIndex}][${props.formName}Charges][${key2}][Discount]']`
								).prop("readonly", true);
							}, 100);
						}
				...
				if (updateChargesFillData.length >= 1) {
					...
					$.each(updateChargesFillData, function (key, value) {
						if (props.containerIndex == key) {
							...
							var ChargesOptions = [];
							$.each(
								value[`Select${props.formName}Charges`],
								function (key2, value2) {
									if (value2.portCode) {
										ChargesOptions.push({
							...
							$.each(
								...
								function (key2, value2) {
											...
											if (value2.BillTo) {
												if (value2.billTo.portCode) {
													...
													tempCharges[key3]["options"] = [
														{
															value: value2.BillTo,
															label: value2.billTo.BranchCode,
														},
													];
												}
											}
									...
									setTimeout(() => {
										$(
											`input[name='${props.formName}HasContainerType[${props.containerIndex}][${props.formName}Charges][${key2}][${props.formName}ChargesUUID]']`
										)
											.closest("tr")
										...
										if (value2.ChargesCodeBreakDown == 1) {
											$(
												`input[name='${props.formName}HasContainerType[${props.containerIndex}][${props.formName}Charges][${key2}][ChargesCodeBreakDown]']`
											)
												.prev()
												.prop("checked", true);
										}
										if (value2.PricingBreakDown == 1) {
											$(
												`input[name='${props.formName}HasContainerType[${props.containerIndex}][${props.formName}Charges][${key2}][PricingBreakDown]']`
											)
												.prev()
												.prop("checked", true);
										}
									...
									if (props.formName == "Quotation") {
										...
										if (
											...
											setTimeout(() => {
												// after append
												$(
													`input[name='${props.formName}HasContainerType[${props.containerIndex}][${props.formName}Charges][${key2}][Qty]']`
												).val(value.Qty);
												setValue(
													`${props.formName}HasContainerType[${props.containerIndex}][${props.formName}Charges][${key2}][Qty]']`,
													value.Qty
												);
											}, 500);
										}
													...
													<>
																				...
																				<input
																					...
																					{...register(
																						`${props.formName}HasContainerType` +
																							...
																							props.containerIndex +
																							"]" +
																							`[${props.formName}Charges][${index}][${props.formName}ChargesUUID]`
																					)}></input>
																				<div className='row'>
																					{props.barge ? (
																						<>
														...
														<tr className={"NestedChargesTR d-none"}>
															<NestedTableNestedCharges
																...
																removeStateRerender={props.removeStateRerender}
