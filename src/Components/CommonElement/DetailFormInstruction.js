...
function DetailFormInstruction(props) {
	...
	useEffect(() => {
		setPodPortCodes(globalContext.podPortCodes);
	}, [globalContext.podPortCodes]);

	useEffect(() => {
		console.log("Access POD PORTS", podPortCodes);
	}, [podPortCodes]);
	...
	function FindBargeNameDetail(val) {
		if (val) {
			getVesselById(val.value, globalContext).then((res) => {
				if (res.data) {
					props.setValue(
						`${props.InstructionItem.formName}[BargeName]`,
						res.data.VesselName
					);
							...
							$.each(value.voyageSchedules, function (key2, value2) {
								if (getPolPortcode == value2.PortCode) {
									if (countPOL == repeatedVoyage) {
										props.setValue(
											`${formName}[ClosingDateTime]`,
											value2.ClosingDateTime
										);
										...
										props.setValue(
											`${formName}[POLLocationCode]`,
											value2.LocationCode
										);
										props.setValue(
											`${formName}[VoyagePOL]`,
											value2.VoyageScheduleUUID
										);
										onChangeTerminalCode(
											{value: value2.LocationCode},
								...
								if (
									getPodPortcode == value2.PortCode &&
									foundPOL &&
									foundPOD == false
								) {
	...
	function VoyageNumOnChangeHandle(data) {
		var index = $("#dynamicmodel-voyagenum")
			.parent()
			.parent()
			.parent()
			.parent()
			.find(".transhipmentQuickForm")
			.children()
			.last()
			.index();
		if (data) {
			...
			$(`#${formNameLowerCase}hastranshipment-` + index + "-tovoyagenum")
				.val(value)
			...
			if (value != "") {
				...
				if (index <= 0) {
					...
					startingVesselName = $(
						`#${formNameLowerCase}hastranshipment-0-fromvesselname`
					).val();
				} else {
					starting = $(".transhipmentQuickForm")
						.last()
						.find(".QuickFormPortCode")
						.find("input:hidden")
						.val();
					startingIndex = index - 1;
					startingVesselName = $(
						`#${formNameLowerCase}hastranshipment-` +
							startingIndex +
							"-tovesselname"
					).val();
				}
				var PodPortcode = props.getValues("DynamicModel[PODPortCode]");
				var PotPortcode = $(".transhipmentQuickForm")
					.last()
					.find(".QuickFormPortCode")
					.find("input:hidden")
					.val();
				...
				getVoyageByIdSpecial(result, globalContext).then((data) => {
					...
					props.setValue(
						`${formName}HasTranshipment[${index}][optionToVoyage]`,
						VoyageArray
					);
					formContext.update(formContext.fields);
					props.setValue(
						`${formName}HasTranshipment[${index}][ToVoyageNum]`,
						data[0]["VoyageUUID"]
					);
					$(
						`#${formNameLowerCase}hastranshipment-` + index + "-tovoyagename"
					).val(data[0]["VoyageNumber"]);
					$(
						`#${formNameLowerCase}hastranshipment-` + index + "-tovesselcode"
					).val(data[0]["vessel"]["VesselCode"]);
					$(
						`#${formNameLowerCase}hastranshipment-` + index + "-tovesselname"
					).val(data[0]["vessel"]["VesselName"]);
					if ($(".transhipment-item").length < 1) {
					...
					$.each(data[0]["voyageSchedules"], function (key, value) {
						...
						if (value.PortCode == PotPortcode) {
							if (countPOT == repeatedVoyage) {
								...
								props.setValue(
									`${formName}HasTranshipment[${index}][LoadingDate]`,
									value.ETA
								);
								//   $(`#${formNameLowerCase}hastranshipment-` + index + "-loadingdate").val(value.ETA).trigger('change.select2')
								props.setValue(
									`${formName}HasTranshipment[${index}][ToVoyagePOT]`,
									value.VoyageScheduleUUID
								);
							...
							if (!value.LocationCode) {
								...
								$.each(value.terminalSelection, function (key1, value1) {
									if (value1.PortDetailsUUID == value.LocationCode) {
										...
										props.setValue(
											`${formName}[PODLocationName]`,
											value1.LocationCode
										);
										props.setValue(
											`${formName}[PODAgentROC]`,
											value1.handlingCompany.AgentCode
										);
										props.setValue(
											`${formName}[PODAgentName]`,
											value1.handlingCompany.CompanyUUID
										);
										props.setValue(
											`${formName}[PODHandlingOfficeCode]`,
											value1.handlingCompanyBranch.CompanyBranchUUID
										);
										props.setValue(
											`${formName}[PODHandlingOfficeName]`,
											value1.handlingCompanyBranch.BranchName
										);
									}
								});
					...
					if (ToVesselName == startingVesselName) {
						...
						props.setValue(
							`${formName}HasTranshipment[${index}][DischargingDate]`,
							""
						);
					}
				});
	...
	useEffect(() => {
		setTimeout(() => {
			props.setValue(
				`${props.InstructionItem.formName}[POLPortTerm]`,
				formContext.defaultPortTerm
			);
			props.setValue(
				`${props.InstructionItem.formName}[PODPortTerm]`,
				formContext.defaultPortTerm
			);
			props.setValue(
				`${props.InstructionItem.formName}[FinalDestinationPortTerm]`,
				formContext.defaultPortTerm
			);
		}, 500);
		...
		<>
			{props.InstructionItem.instructionList.map((res, index) => {
				...
				var POLElement = [
					{
						type: "hidden",
						id: `${lowercaseFormName}-voyage-pol`,
						name: `${formName}[VoyagePOL]`,
					},
					{
						title: "Port Code",
						id: `${lowercaseFormName}-${lowercaseInstructionType}portcode`,
						className: `reflect-field getTerminalPortCode`,
						name: `${formName}[${instructionType}PortCode]`,
						dataTarget: `${instructionType}PortCode-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: props.port,
						onChange: onChangePortCode,
						specialFeature: [],
					},
					{
						title: "Area",
						id: `${lowercaseFormName}-${lowercaseInstructionType}areaname`,
						className: `reflect-field AreaName OriReadOnlyClass`,
						name: `${formName}[${instructionType}AreaName]`,
						dataTarget: `${instructionType}AreaName-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "255",
						onChange: "",
					...
					{
						title: "Terminal Code",
						id: `${lowercaseFormName}-${lowercaseInstructionType}locationcode`,
						className: `reflect-field`,
						name: `${formName}[${instructionType}LocationCode]`,
						dataTarget: `${instructionType}LocationCode-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.optionPOLTerminal,
						onChange: onChangeTerminalCode,
						specialFeature: [],
					},
					{
						title: "Terminal Name",
						id: `${lowercaseFormName}-${lowercaseInstructionType}locationname`,
						className: `reflect-field OriReadOnlyClass`,
						name: `${formName}[${instructionType}LocationName]`,
						dataTarget: `${instructionType}LocationName-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "255",
						onChange: "",
					...
					{
						title: "Port Term",
						id: `${lowercaseFormName}-${lowercaseInstructionType}portterm`,
						className: `reflect-field`,
						name: `${formName}[${instructionType}PortTerm]`,
						dataTarget: `${instructionType}PortTerm-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.portTerm,
					...
					{
						title: "Freight Term",
						id: `${lowercaseFormName}-${lowercaseInstructionType}freightterm`,
						className: `reflect-field`,
						name: `${formName}[${instructionType}FreightTerm]`,
						dataTarget: `${instructionType}FreightTerm-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.freightTerm,
						onChange: "",
						specialFeature: [],
					},
					{
						title: "Terminal Handler ROC",
						id: `${lowercaseFormName}-${lowercaseInstructionType}handlingroc`,
						className: `reflect-field OriReadOnlyClass`,
						name: `${formName}[${instructionType}AgentROC]`,
						dataTarget: ``,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "255",
					...
					},
					{
						...
						id: `${lowercaseFormName}-${lowercaseInstructionType}handlingcompany`,
						className: `reflect-field`,
						name: `${formName}[${instructionType}AgentName]`,
						dataTarget: ``,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.optionPOLAgentCompany,
						onChange: `onChange${instructionType}HandlingCompany`,
						specialFeature: [],
					},
					{
						title: "Terminal Handler Branch Code",
						id: `${lowercaseFormName}-${lowercaseInstructionType}handlingofficecode`,
						className: `reflect-field`,
						name: `${formName}[${instructionType}HandlingOfficeCode]`,
						dataTarget: `${instructionType}HandlingOffice-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.optionPOLAgentCompanyBranch,
						maxlength: "255",
						onChange: `onChange${instructionType}HandlingOfficeCode`,
						specialFeature: [],
					},
					{
						title: "Terminal Handler Branch Name",
						id: `${lowercaseFormName}-${lowercaseInstructionType}handlingofficename`,
						className: `reflect-field`,
						name: `${formName}[${instructionType}HandlingOfficeName]`,
						dataTarget: ``,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "255",
					...
					{
						title: "Req ETA",
						id: `${lowercaseFormName}-${lowercaseInstructionType}reqeta`,
						className: `reflect-field flatpickr-input-time flatpickr-input OriReadOnlyClass ${
							props.InstructionItem.formName == "BillOfLading" ||
							props.InstructionItem.formName == "BillOfLadingBarge"
								? "pointerEventsStyle"
								: ""
						}`,
						name: `${formName}[${instructionType}ReqETA]`,
						dataTarget: `${lowercaseInstructionType}reqeta`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
						maxlength: "255",
						value: formContext.pOLReqETA,
						onChange: "",
						specialFeature: ["readOnly"],
					},
				];
				...
				var PODElement = [
					{
						type: "hidden",
						id: `${lowercaseFormName}-voyage-pod`,
						name: `${formName}[VoyagePOD]`,
					},
					{
						title: "Port Code",
						id: `${lowercaseFormName}-${lowercaseInstructionType}portcode`,
					...
					},
					{
						...
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.optionPODTerminal,
						onChange: onChangeTerminalCode,
						specialFeature: [],
					},
					{
					...
					},
					{
						...
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.optionPODAgentCompany,
						onChange: `onChange${instructionType}HandlingCompany`,
						specialFeature: [],
					},
					{
						...
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.optionPODAgentCompanyBranch,
						maxlength: "255",
						onChange: `onChange${instructionType}HandlingOfficeCode`,
						specialFeature: [],
					},
				...
				var FinalDestinationElement = [
					{
						title: "Port Code",
						id: `${lowercaseFormName}-${lowercaseInstructionType}`,
						className: `reflect-field getTerminalPortCode`,
						name: `${formName}[${instructionType}]`,
						dataTarget: `${instructionType}-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-12",
						type: "dropdown",
						option: props.port,
						onChange: onChangeFinalDestinationPortCode,
						specialFeature: [],
					},
					{
						title: "Area",
						id: `${lowercaseFormName}-${lowercaseInstructionType}areaname`,
						className: `reflect-field AreaName`,
						name: `${formName}[${instructionType}Area]`,
						dataTarget: `${instructionType}Area-ShippingInstruction`,
						gridSize: "col-xs-12 col-md-12",
						type: "input-text",
						maxlength: "255",
						onChange: "",
					...
					},
					{
						...
						className: `reflect-field`,
						name: `${formName}[${instructionType}PortTerm]`,
						dataTarget: `FDPortTerm-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-12",
						type: "dropdown",
						option: formContext.portTerm,
						onChange: "",
					...
					{
						title: "Agent Company",
						id: `${lowercaseFormName}-${lowercaseInstructionType}handler`,
						className: `reflect-field`,
						name: `${formName}[${instructionType}Handler]`,
						dataTarget: `${instructionType}Handler-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-12",
						type: "dropdown-asyncSelect",
						value: formContext.defaultFinalDestinationCompany,
						onChange: formContext.setDefaultFinalDestinationCompany,
						specialFeature: [],
					},
				];
				var VoyageElement = [
					{
						title: "Voyage Num",
						id: `${lowercaseFormName}-voyagenum`,
						className: `reflect-field AllScheduleVoyage`,
						name: `${formName}[VoyageNum]`,
						dataTarget: `VoyageNum-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.VoyageNum,
						onChange: FindVoyageNumberDetail,
						specialFeature: ["VoyageTools"],
					},
					{
						title: "Voyage Name",
						id: `${lowercaseFormName}-voyagename`,
						className: `reflect-field OriReadOnlyClass`,
						name: `${formName}[VoyageName]`,
						dataTarget: `VoyageName-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "255",
						onChange: "",
					...
					{
						title: "Vessel Code",
						id: `${lowercaseFormName}-vesselcode`,
						className: `reflect-field OriReadOnlyClass`,
						name: `${formName}[VesselCode]`,
						dataTarget: `VesselCode-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "45",
						onChange: "",
					...
					{
						title: "Vessel Name",
						id: `${lowercaseFormName}-vesselname`,
						className: `reflect-field OriReadOnlyClass`,
						name: `${formName}[VesselName]`,
						dataTarget: `VesselName-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "255",
						onChange: "",
					...
					{
						title: "POL ETA",
						id: `${lowercaseFormName}-poleta`,
						className: `reflect-field flatpickr-input-time flatpickr-input pointerEventsStyle OriReadOnlyClass`,
						name: `${formName}[POLETA]`,
						dataTarget: `poleta`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
						onChange: "",
						specialFeature: ["readOnly", "disabled"],
					},
					{
						title: "POL ETD",
						id: `${lowercaseFormName}-poletd`,
						className: `reflect-field flatpickr-input-time flatpickr-input pointerEventsStyle OriReadOnlyClass`,
						name: `${formName}[POLETD]`,
						dataTarget: `poletd`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
						onChange: "",
					...
					{
						title: "POL SCN Code",
						id: `${lowercaseFormName}-polscncode`,
						className: `reflect-field`,
						name: `${formName}[POLSCNCode]`,
						dataTarget: `POLSCNCode-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "255",
						onChange: "",
					...
					},
					{
						...
						id: `${lowercaseFormName}-closingdatetime`,
						className: `reflect-field closingDateTime flatpickr-input-time flatpickr-input pointerEventsStyle OriReadOnlyClass`,
						name: `${formName}[ClosingDateTime]`,
						dataTarget: `closingDateTime`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
						onChange: "",
						specialFeature: ["readOnly", "disabled"],
					...
					{
						title: "POD ETA",
						id: `${lowercaseFormName}-podeta`,
						className: `reflect-field flatpickr-input-time flatpickr-input pointerEventsStyle OriReadOnlyClass`,
						name: `${formName}[PODETA]`,
						dataTarget: `podeta`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
						onChange: "",
					...
					{
						title: "POD ETD",
						id: `${lowercaseFormName}-podetd`,
						className: `reflect-field flatpickr-input-time flatpickr-input pointerEventsStyle OriReadOnlyClass`,
						name: `${formName}[PODETD]`,
						dataTarget: `podetd`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
						onChange: "",
					...
					{
						title: "POD SCN Code",
						id: `${lowercaseFormName}-podscncode`,
						className: `reflect-field`,
						name: `${formName}[PODSCNCode]`,
						dataTarget: `PODSCNCode-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "255",
						onChange: "",
					...
					{
						title: "Ship Op ROC",
						id: `CompanyROC-${instructionType}-DetailForm`,
						className: `dropdownInputCompany reflect-field ShipOpROC`,
						name: `${formName}[ShipOperator]`,
						dataTarget: `CompanyROC-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
					...
					{
						title: "Ship Op Company Name",
						id: `${lowercaseFormName}-shipoperatorcompany`,
						className: `reflect-field OriReadOnlyClass`,
						name: `${formName}[ShipOperatorCompany]`,
						dataTarget: `CompanyName-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						onChange: "",
						specialFeature: ["readOnly"],
					},
					{
						...
						id: `BranchCode-${instructionType}-DetailForm`,
						className: `dropdownInputBranch reflect-field ShipOpBranchCode`,
						name: `${formName}[ShipOperatorBranchCode]`,
						dataTarget: `BranchCode-${instructionType}`,
						dataRefer: `${lowercaseFormName}-shipoperator`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-dropdownInputBranch",
						onChange: "",
						specialFeature: [],
					},
					{
						title: "Ship Op Branch Name",
						id: `${lowercaseFormName}-shipoperatorbranchname`,
						className: `reflect-field OriReadOnlyClass`,
						name: `${formName}[ShipOperatorBranchName]`,
						dataTarget: `BranchName-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						onChange: "",
						specialFeature: ["readOnly"],
					...
					{
						title: `Auto Billing`,
						id: `${lowercaseFormName}-autobilling`,
						className: `mb-1 ml-1`,
						name: `${formName}[AutoBilling]`,
						dataTarget: ``,
						gridSize: "col-xs-12 col-md-12",
						type: "checkbox",
						onChange: "",
					...
					},
					{
						...
						id: `${lowercaseFormName}-insisttranshipment`,
						className: `mb-1 ml-1`,
						name: `${formName}[InsistTranshipment]`,
						dataTarget: ``,
						gridSize: "col-xs-12 col-md-12",
						type: "checkbox",
						onChange: "",
					...
					{
						title: `Apply D&D`,
						id: `${lowercaseFormName}-applydnd`,
						className: `mb-1 ml-1`,
						name: `${formName}[ApplyDND]`,
						dataTarget: ``,
						gridSize: "col-xs-12 col-md-12",
						type: "checkbox",
						checkBoxClass: "DND",
						onChange: "",
						specialFeature: ["defaultCheck"],
					},
					{
						title: `D&D Combined`,
						id: `${lowercaseFormName}-dndcombined`,
						className: `mb-1 ml-1`,
						name: `${formName}[DNDCombined]`,
						dataTarget: ``,
						gridSize: "col-xs-12 col-md-12 ml-5 DNDCombined",
						type: "checkbox",
						checkBoxClass: "DND",
						onChange: "",
						specialFeature: ["defaultCheck"],
				...
				var VoyageElementNoDND = [
					...
					},
					{
						title: "POL ETA",
						id: `${lowercaseFormName}-poleta`,
						className: `reflect-field flatpickr-input-time flatpickr-input pointerEventsStyle`,
						name: `${formName}[POLETA]`,
						dataTarget: `poleta`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
					...
					},
					{
						title: "Closing Date Time",
						id: `${lowercaseFormName}-closingdatetime`,
						className: `reflect-field closingDateTime flatpickr-input-time flatpickr-input pointerEventsStyle`,
						name: `${formName}[ClosingDateTime]`,
						dataTarget: `closingDateTime`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
				...
				var VoyageElementBarge = [
					{
						title: "Voyage Num",
						id: `${lowercaseFormName}-voyagenum`,
						className: `reflect-field readOnlySelect AllScheduleVoyage bargeRelatedField`,
						name: `${formName}[VoyageNum]`,
						dataTarget: `VoyageNum-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
					...
					},
					{
						title: "Voyage Name",
						id: `${lowercaseFormName}-voyagename`,
						className: `reflect-field OriReadOnlyClass bargeRelatedField`,
						name: `${formName}[VoyageName]`,
						dataTarget: `VoyageName-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
					...
					{
						title: "Barge Code",
						id: `${lowercaseFormName}-bargecode`,
						className: `reflect-field readOnlySelect bargeRelatedField`,
						name: `${formName}[BargeCode]`,
						dataTarget: `BargeCode-ShippingInstructions`,
						gridSize: "col-xs-12 col-md-6",
						type: "dropdown",
						option: formContext.bargeCode,
						onChange: "",
						specialFeature: ["readOnly"],
					},
					{
						title: "Barge Name",
						id: `${lowercaseFormName}-bargename`,
						className: `reflect-field OriReadOnlyClass bargeRelatedField`,
						name: `${formName}[BargeName]`,
						dataTarget: `BargeName-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
						maxlength: "45",
						onChange: "",
					...
					},
					{
						title: "POL ETA",
						id: `${lowercaseFormName}-poleta`,
						className: `reflect-field flatpickr-input-time flatpickr-input pointerEventsStyle OriReadOnlyClass bargeRelatedField`,
						name: `${formName}[POLETA]`,
						dataTarget: `poleta`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
					...
					},
					{
						title: "POL SCN Code",
						id: `${lowercaseFormName}-polscncode`,
						className: `reflect-field bargeRelatedField`,
						name: `${formName}[POLSCNCode]`,
						dataTarget: `POLSCNCode-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-text",
					...
					},
					{
						title: "Closing Date Time",
						id: `${lowercaseFormName}-closingdatetime`,
						className: `reflect-field closingDateTime flatpickr-input-time flatpickr-input pointerEventsStyle OriReadOnlyClass bargeRelatedField`,
						name: `${formName}[ClosingDateTime]`,
						dataTarget: `closingDateTime`,
						gridSize: "col-xs-12 col-md-6",
						type: "flatpickr-input-time",
					...
					},
					{
						title: "Ship Op ROC",
						id: `CompanyROC-${instructionType}-DetailForm`,
						className: `dropdownInputCompany reflect-field ShipOpROC bargeRelatedField`,
						name: `${formName}[ShipOperator]`,
						dataTarget: `CompanyROC-${instructionType}`,
						gridSize: "col-xs-12 col-md-6",
						type: "input-dropdownInputCompany",
					...
					},
					{
						title: "Ship Op Branch Code ",
						id: `BranchCode-${instructionType}-DetailForm`,
						className: `dropdownInputBranch reflect-field ShipOpBranchCode bargeRelatedField`,
						name: `${formName}[ShipOperatorBranchCode]`,
						dataTarget: `BranchCode-${instructionType}`,
						dataRefer: `${lowercaseFormName}-shipoperator`,
						gridSize: "col-xs-12 col-md-6",
				...
				var VoyageElementBargeDO = [
					...
					},
					{
						title: `D&D Combined`,
						id: `${lowercaseFormName}-dndcombined`,
						className: `mb-1 ml-1 bargeRelatedField`,
						name: `${formName}[DNDCombined]`,
						dataTarget: ``,
						gridSize: "col-xs-12 col-md-12 ml-5 DNDCombined",
						type: "checkbox",
				...
				return (
					<div
						key={index}
						className={`${lowercaseFormName}-${lowercaseInstructionType}-form`}>
						<div className='card lvl2'>
							<div className='card-header'>
								<h3 className='card-title'>
									{res === "FinalDestination" ? SpaceFormName : res}{" "}
								</h3>
							...
							<div className='card-body'>
								<div className='row'>
									...
									{instructionType == "POD" &&
										podPortCodes.map((v, ind) => (
											<>
												<h6 className='col-lg-12 mb-3'>POD-{v.label}</h6>
									...
									<BuildUIForInstruction
										...
										element={
											instructionType == "POL"
												? POLElement
												: instructionType == "FinalDestination"
												? FinalDestinationElement
												: props.InstructionItem.formName ==
												  "ContainerReleaseOrder"
												? VoyageElementNoDND
												: props.InstructionItem.formNameBarge ==
														"SalesCreditNoteBarge" ||
												  props.InstructionItem.formNameBarge ==
														"SalesDebitNoteBarge" ||
												  props.InstructionItem.formNameBarge ==
														"SalesInvoiceBarge"
				...
				return res.type === "input-text" ? (
					...
					<div key={index} className={res.gridSize}>
						<div className='form-group'>
							...
							<div className='input-group'>
								<Controller
									...
									render={({field: {onChange, value}}) => (
										<Select
											...
											styles={
												props.props.verificationStatus
								...
								/>
								{res.specialFeature.includes("VoyageTools") ? (
									<div
					...
					<div key={index} className={res.gridSize}>
						<div className='form-group'>
							...
							<input
								type='hidden'
								id={`${lowercaseFormName}-shipoperator`}
								className={`form-control`}
								{...props.props.register(name)}
								readOnly='readOnly'
								data-target={`CompanyID-${instructionType[props.cardIndex]}`}
							/>
							<input
								defaultValue={res.defaultValue}
								id={res.id}
							...
							<div
								...
								className={`dropdownTable ${lowercaseFormName}${instructionType[
									props.cardIndex
								].toLowerCase()}-dropdown d-none`}>
								<table
									id={`CompanyROC-${
										instructionType[props.cardIndex]
									}Hauler-DetailForm-Table`}></table>
							</div>
						</div>
					</div>
					...
					<div key={index} className={res.gridSize}>
						<div className='form-group'>
							...
							<input
								type='hidden'
								id={`${lowercaseFormName}-shipoperatorbranchcode`}
								className={`form-control`}
								{...props.props.register(name)}
								readOnly='readOnly'
								data-target={`BranchID-${instructionType[props.cardIndex]}`}
							/>
							<input
								defaultValue={res.defaultValue}
								id={res.id}
					...
					<div key={index} className={res.gridSize}>
						<div className='form-group'>
							...
							<div className='input-group'>
								...
								<div
									className='input-group-append openAttentionModal'
									data-refer={`${lowercaseFormName}${instructionType[
										props.cardIndex
									].toLowerCase()}-branchcode`}>
					...
					<div key={index} className={res.gridSize}>
						<div
							className={`form-group field-${lowercaseFormName}-${res.title
								.toLowerCase()
								.replace(" ", "")}`}>
							<input
								type={"checkbox"}
								className={`mt-1 ${res.checkBoxClass ? res.checkBoxClass : ""}`}
								id={res.id}
							...
							/>
							<label htmlFor={res.id} className={res.className}>
								{res.title}
							</label>
						</div>
					</div>
					...
					<div key={index}>
						<div className='col-md-12 ml-5'>
							<div className='col-md-8 DNDCombineDay ml-2'>
								...
								<input
									type='checkbox'
									className='CombineDayCheckBox ml-2 checkbox-inline mb-1 d-none'
								/>
								<div
									className={`form-group field-${lowercaseFormName}-dndcombinedday`}>
									<input
										type='number'
										id={`${lowercaseFormName}-dndcombinedday`}
										className='form-control reflect-field'
										{...props.props.register(`${formName}[DNDCombinedDay]`)}
										defaultValue='10'
										data-target='DNDCombinedDay'
									/>
								</div>
							</div>
						</div>
						<div className='col-md-12 ml-5'>
							<div className='row'>
								<div className='col-md-5 Detention ml-2 d-none'>
									...
									<input
										type='checkbox'
										className='DetentionCheckBox  ml-2 checkbox-inline mb-1 d-none'
									/>
									<div
										className={`form-group field-${lowercaseFormName}-detention`}>
										<input
											type='number'
											id='quotation-detention'
											className='form-control reflect-field'
											{...props.props.register(`${formName}[Detention]`)}
											defaultValue='5'
											data-target='Detention'
										/>
									</div>
								</div>
								...
								<div className='col-md-5 Demurrage ml-2 d-none'>
									...
									<input
										type='checkbox'
										className='DemurrageCheckBox ml-2 checkbox-inline mb-1 d-none'
									/>
									<div
										className={`form-group field-${lowercaseFormName}-demurrage`}>
										<input
											type='number'
											id='quotation-demurrage'
											className='form-control reflect-field'
											{...props.props.register(`${formName}[Demurrage]`)}
											defaultValue='5'
											data-target='Demurrage'
										/>
									</div>
								</div>
							</div>
