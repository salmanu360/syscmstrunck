...
function QuickFormContainer(props) {
  ...
  useEffect(() => {
		...
		globalContext.setPodPortCodes(selectedValue);
	}, [selectedValue]);
	...
	const {fields, append, prepend, remove, swap, update, move, insert, replace} =
		useFieldArray({
			control,
			name: `${formName}HasContainerType`,
		});
									...
									? selectedValue?.map((item, index) => {
											return (
												<div
													...
													<button
														className='remove-btn sss border-0 bg-transparent text-danger p-1'
