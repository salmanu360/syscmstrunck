...
function QuickFormContainer(props) {
    ...
    if (formName == "Quotation") {
        defaultValueObject = { QuotationHasContainerType: [{ "Name": `${formName}HasContainerType`, "Qty": 1, "ContainerItem": ContainerColumn, ContainerTypeOptions: props.containerType }] }
    } else if (formName == "BookingReservation") {
        defaultValueObject = { BookingReservationHasContainerType: [{ "Name": `${formName}HasContainerType`, "Qty": 1, "ContainerItem": ContainerColumn, ContainerTypeOptions: props.containerType }] }
    } else if (formName == "SalesInvoice") {
        defaultValueObject = { SalesInvoiceHasContainerType: [{ "Name": `${formName}HasContainerType`, "Qty": 1, "ContainerItem": ContainerColumn, ContainerTypeOptions: props.containerType }] }
    }
    else {
        defaultValueObject = { BookingConfirmationHasContainerType: [{ "Name": `${formName}HasContainerType`, "Qty": 1, "ContainerItem": ContainerColumn, ContainerTypeOptions: props.containerType }] }
    }
    ...
    function appendContainerHandle() {
        append({ "Name": `${formName}HasContainerType`, "Qty": 1, "ContainerItem": ContainerColumn, ContainerTypeOptions: props.containerType })
    ...
    function getContainerCodeByContainerType(val, index) {
        ...
        if (val) {
            ...
            getContainerTypeById(val.value, globalContext).then(data => {
                setValue(`${formName}HasContainerType[${index}][Length]`, data.data.Length)
                setValue(`${formName}HasContainerType[${index}][Width]`, data.data.Width)
                setValue(`${formName}HasContainerType[${index}][Height]`, data.data.Height)
        ...
        if (!props.transferPartial) {
            if (fields.length > 0) {
                ...
                $.each(fields, function (key, value) {
                    ...
                    getChargesByContainerTypeAndPortCode($(`input[name='${formName}HasContainerType[${key}][ContainerType]']`).val(), UserPortCode, globalContext, VoyageUUID, false).then(data => {
    ...
    $(".onChangeContainerType").unbind().on("change", function (e) {
        setTimeout(() => {
            ...
            if (label.includes("RF")) {
                $(`input[name='${formName}HasContainerType[${index}][Temperature]']`).attr('readonly', false);
                $.each(fields[index]["ContainerItem"], function (key2, value2) {
                    if (value2.name == "Temperature") {
                        setValue(`${formName}HasContainerType[${index}][ContainerItem][${key2}][requiredField]`, true)
                        update(fields)
                    }
                })
            } else {
                $(`input[name='${formName}HasContainerType[${index}][Temperature]']`).val("")
                $(`input[name='${formName}HasContainerType[${index}][Temperature]']`).attr('readonly', true);
                $.each(fields[index]["ContainerItem"], function (key2, value2) {
                    if (value2.name == "Temperature") {
                        setValue(`${formName}HasContainerType[${index}][ContainerItem][${key2}][requiredField]`, false)
                        update(fields)
                    }
                })
            }
            ...
            var filters = {
                "OwnershipType": getValues(`${formName}HasContainerType[${index}][OwnershipType]`),
            ...
            getContainers(value, filters, globalContext).then(data => {
                ...
                setValue(`${formName}HasContainerType[${index}]["ContainerOptions"]`, sortArray(optionContainerList))
                update(fields)
            })
        }, 100);
    ...
    function getCOCCompany(val, index) {
        $(`input[name='${formName}HasContainerType[${index}][BoxOwnership]']`).parent().trigger("change")
        setGetErrorsFieldsUpdate(val)
    }
    ...
    $(".onChangeOwnership").unbind().on("change", function (e) {
        setTimeout(() => {
            ...
            if (value) {
                            ...
                            if (value.Valid != 0) {
                                ...
                                $.each(fields[index]["ContainerItem"], function (key2, value2) {
                                    if (value2.columnName == "Box Op Code") {
                                        setValue(`${formName}HasContainerType[${index}][ContainerItem][${key2}][optionColumn]`, Box_OpCompany)
                                        update(fields)
                                    }
                                    if (value2.columnName == "Box Op Branch Code") {
                                        setValue(`${formName}HasContainerType[${index}][ContainerItem][${key2}][optionColumn]`, sortArray(BoxOperatorBranch))
                                        update(fields)
                                    }
                                })
                                setValue(`${formName}HasContainerType[${index}][BoxOperatorName]`, BoxOperatorName)
                                setValue(`${formName}HasContainerType[${index}][BoxOperatorBranch]`, defaultBoxOperatorBranch)
                                setValue(`${formName}HasContainerType[${index}][BoxOperatorBranchName]`, defaultBoxOperatorBranchName)
                            }
                        })
                    })
                }
                ...
                var filters = {
                    "OwnershipType": value,
                    "Container.ContainerType": getValues(`${formName}HasContainerType[${index}][ContainerType]`),
                    "Container.Status": "Available"
                };
                getContainers(value, filters, globalContext).then(data => {
                    try {
                ...
                $.each(fields[index]["ContainerItem"], function (key2, value2) {
                    if (value2.columnName == "Box Op Code") {
                        setValue(`${formName}HasContainerType[${index}][ContainerItem][${key2}][optionColumn]`, [])
                        update(fields)
                    }
                    if (value2.columnName == "Box Op Branch Code") {
                        setValue(`${formName}HasContainerType[${index}][ContainerItem][${key2}][optionColumn]`, [])
                ...
                setValue(`${formName}HasContainerType[${index}][BoxOperatorName]`, "")
                setValue(`${formName}HasContainerType[${index}][BoxOperatorBranch]`, "")
                setValue(`${formName}HasContainerType[${index}][BoxOperatorBranchName]`, "")
            }
        }, 100);
    })

    $(".onChangeBoxOperator").off("change").on("change", function () {
        setTimeout(() => {
            var Findtrindex = $(this).closest("tr").index()
            var index = Findtrindex / 2
            var value = $(this).find("input:hidden").val()
    ...
    function getBoxOperatorBranchByBoxOperatorCompany(val, index) {
        ...
        $(`input[name='${formName}HasContainerType[${index}][BoxOperator]']`).parent().trigger("change")
    }
    ...
    function onChangeUNNumber(val, index) {
        ...
        $.each(data["ContainerItem"], function (key2, value2) {
            if (value2.columnName == "UN Number") {
                setValue(`${formName}HasContainerType[${index}][ContainerItem][${key2}][optionColumn]`, val)
                update(fields)
            }
        })
        if (val) {
            ...
            getUNNumberByID(UNNumber, globalContext).then(data => {
                setValue(`${formName}HasContainerType[${index}][DGClass]`, data.data.Class)
            })
        } else {
    ...
    function onChangeHSCode(val, index) {
        ...
        if (val) {
            ...
            getHSCodeByID(HSCode, globalContext).then(data => {
                setValue(`${formName}HasContainerType[${index}][Commodity]`, data.data.Description)
            })
        } else {
    ...
    function getBoxOperatorBranchName(val, index) {
        if (val) {
            GetBranchData(val.value, globalContext).then(data => {
                setValue(`${formName}HasContainerType[${index}][BoxOperatorBranchName]`, data.data.BranchName)
            })
        } else {
            setValue(`${formName}HasContainerType[${index}][BoxOperatorBranchName]`, "")
        }
    ...
    useEffect(() => {
        setValue(`${formName}HasContainerType[${totalDiscount.containerIndex}][TotalDiscount]`, totalDiscount.TotalDiscountTwoDecimal)
        setValue(`${formName}HasContainerType[${totalTax.containerIndex}][TotalTax]`, totalTax.TotalTaxTwoDecimal)
        setValue(`${formName}HasContainerType[${totalAmount.containerIndex}][Total]`, totalAmount.TotalAmountTwoDecimal)
        ...
        if (updateContainerFillData.length > 0) {
            ...
            $.each(updateContainerFillData, function (key, value) {
                ...
                setTimeout(() => { // after append
                    if (value.Empty == 1) {
                        $(`input[name='${formName}HasContainerType[${key}][Empty]']`).prev().prop("checked", true)
                    }
                    setValue(`${formName}HasContainerType[${key}][ContainerCode][]`, selectedContainer)

                        if (props.userRule) {
                            var viewChargesAccess = props.userRule.find((item) => item == `view-charges-booking-reservation`) !== undefined
                        }
                    
                    $(`input[name='${formName}HasContainerType[${key}][${formName}HasContainerTypeUUID]']`).closest("tr").find(".MarkReadonly").val(value.Mark)
                    $(`input[name='${formName}HasContainerType[${key}][${formName}HasContainerTypeUUID]']`).closest("tr").find(".GoodDescriptionReadonly").val(value.GoodsDescription)
                    if(formName !="SalesInvoice"){
                        if(value[`${formName}Charges`].length >0){
                            if(formName=="BookingReservation"){
                                if(viewChargesAccess){
                                    var charges = $(`input[name='${formName}HasContainerType[${key}][${formName}HasContainerTypeUUID]']`).closest(".container-itemTR").next()
                                    var Icon = $(`input[name='${formName}HasContainerType[${key}][${formName}HasContainerTypeUUID]']`).closest(".container-itemTR").find(".ChargesDisplay").children()[0]
                                                                        ...
                                                                        item.ContainerItem.map((item2, index2) => {
                                                                            if (item2.inputType == "input") {
                                                                                return (
                                                                                    <td className={item2.class}>
                                                                                        {item2.requiredField ?
                                                                                            <input defaultValue='' readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']', { required: "required" })} className={`form-control ${item2.fieldClass ? item2.fieldClass : ""} ${errors[`${formName}HasContainerType`] ? errors[`${formName}HasContainerType`][`${index}`] ? errors[`${formName}HasContainerType`][`${index}`][`${item2.name}`] ? "has-error" : "" : "" : ""}`} />
                                                                                            :
                                                                                            <input defaultValue='' readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']')} className={`form-control ${item2.fieldClass ? item2.fieldClass : ""}`} />
                                                                                        }
                                                                            ...
                                                                            if (item2.inputType == "number") {
                                                                                return (
                                                                                    <td className={item2.class}>
                                                                                        {item2.requiredField ?
                                                                                            <input type="number" defaultValue='' readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']', { required: "required" })} className={`form-control ${item2.fieldClass ? item2.fieldClass : ""} ${errors[`${formName}HasContainerType`] ? errors[`${formName}HasContainerType`][`${index}`] ? errors[`${formName}HasContainerType`][`${index}`][`${item2.name}`] ? "has-error" : "" : "" : ""}`} />
                                                                                            :
                                                                                            <input type="number" defaultValue='' readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']')} className={`form-control ${item2.fieldClass ? item2.fieldClass : ""}`} />
                                                                                        }
                                                                                ...
                                                                                return (
                                                                                    <td className={item2.class}>
                                                                                        <div className="input-group">
                                                                                            ...
                                                                                            {props.transferPartial &&
                                                                                                <input defaultValue='' readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasContainerType` + '[' + index + ']' + '[ContainerArray]')} className={`form-control containerArrayList d-none ${item2.fieldClass ? item2.fieldClass : ""}`} />
                                                                                            }
                                                                                            {item2.requiredField ?
                                                                                                <input type="number" defaultValue={item2.defaultValue} readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']', { required: "required" })} onBlur={(val) => { val ? item2.onBlur(val, index) : item2.onBlur(null, index) }} className={`form-control ${item2.fieldClass ? item2.fieldClass : ""} ${errors[`${formName}HasContainerType`] ? errors[`${formName}HasContainerType`][`${index}`] ? errors[`${formName}HasContainerType`][`${index}`][`${item2.name}`] ? "has-error" : "" : "" : ""}`} />
                                                                                                :
                                                                                                <input type="number" defaultValue={item2.defaultValue} readOnly={item2.readOnly ? item2.readOnly : false} {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']')} onBlur={(val) => { val ? item2.onBlur(val, index) : item2.onBlur(null, index) }} className={`form-control ${item2.fieldClass ? item2.fieldClass : ""}`} />
                                                                                            }
                                                                                        ...
                                                                                        <div className={`SelectContainerCodeField d-none ${props.transferPartial && "ForPartial"}`}>
                                                                                            <Controller
                                                                                                name={(`${formName}HasContainerType` + '[' + index + ']' + '[ContainerCode][]')}
                                                                                                control={control}
                                                                                                render={({ field: { onChange, value } }) => (
                                                                                                    <Select
                                                                                                        ...
                                                                                                        {...register(`${formName}HasContainerType` + '[' + index + ']' + '[ContainerCode][]')}
                                                                                                        value={value
                                                                                                            ? Array.isArray(value)
                                                                                                                ? value.map((c) =>
                                                                            ...
                                                                            if (item2.inputType == "checkbox") {
                                                                                return (
                                                                                    <td className={item2.class} style={{ textAlign: "center", verticalAlign: "middle" }}>
                                                                                        ...
                                                                                        <input type={"hidden"} defaultValue={item2.defaultValue == 1 ? "1" : "0"} className="" {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']')} />
                                                                                    </td>
                                                                                )
                                                                            }
                                                                            if (item2.inputType == "single-select") {
                                                                                if (index2 == 0) {
                                                                                    return (
                                                                                        <td className={item2.class}>
                                                                                            <input type="hidden" {...register(`${formName}HasContainerType[${index}][${formName}HasContainerTypeUUID]`)} className={`${formName}HasContainerTypeUUID`} />
                                                                                            <div className="row">
                                                                                                ...
                                                                                                <div className="col-md-8" style={{ paddingLeft: "0px", paddingRight: "0px" }}>
                                                                                                    {item2.requiredField ?
                                                                                                        <Controller
                                                                                                            name={(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']')}
                                                                                                            control={control}
                                                                                                            render={({ field: { onChange, value } }) => (
                                                                                                                <Select
                                                                                                                
                                                                                                                    isClearable={true}
                                                                                                                    {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']', { required: "P cannot be blank." })}
                                                                                                                    value={value ? item.ContainerTypeOptions.find(c => c.value === value) : null}
                                                                                                                    onChange={val => { val == null ? onChange(null) : onChange(val.value); item2.onChange(val, index); formName == "BookingReservation" ? formContext.QuotationRequiredFields() : val == null ? onChange(null) : onChange(val.value); }}
                                                                                                                    options={item.ContainerTypeOptions}
                                                                                                                    isOptionDisabled={(selectedValue) => selectedValue.selected == true}
                                                                                                                    menuPortalTarget={document.body}
                                                                                                                    className={`basic-single ${item2.fieldClass ? item2.fieldClass : ""}  ${errors[`${formName}HasContainerType`] ? errors[`${formName}HasContainerType`][`${index}`] ? errors[`${formName}HasContainerType`][`${index}`][`${item2.name}`] ? "has-error-select" : "" : "" : ""}`}
                                                                                                                    classNamePrefix="select"
                                                                                                                    onKeyDown={handleKeydown}
                                                                                                                    styles={globalContext.customStyles}
                                                                                                        ...
                                                                                                        <Controller
                                                                                                            ...
                                                                                                            render={({ field: { onChange, value } }) => (
                                                                                                                <Select
                                                                                                                    isClearable={true}
                                                                                                                    {...register(`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']')}
                                                                                                                    value={value ? item.ContainerTypeOptions.find(c => c.value === value) : null}
                                                                                                                    onChange={val => { val == null ? onChange(null) : onChange(val.value); item2.onChange(val, index); formName == "BookingReservation" ? formContext.QuotationRequiredFields() : val == null ? onChange(null) : onChange(val.value); }}
                                                                                                                    options={item.ContainerTypeOptions}
                                                                                                                    isOptionDisabled={(selectedValue) => selectedValue.selected == true}
                                                                                                ...
                                                                                                <div className="modal-content">
                                                                                                    ...
                                                                                                    <div className="modal-body">
                                                                                                        <div className="form-group">
                                                                                                            ...
                                                                                                            <textarea id="" className={`form-control ${item2.modelClass}`} {...register((`${formName}HasContainerType` + '[' + index + ']' + '[' + item2.name + ']'))} rows="5" placeholder={`Enter ${item2.columnName}`}></textarea>
