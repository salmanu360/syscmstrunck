        ...
        {props.DocumentItem.element.map((res, index) => {
                ...
                <div key={index} className={res.gridSize}>
                    <div className="form-group">
                        ...
                        {res.specialFeature.includes("required") ? 
                            <input {...props.register(name,{required: `${res.title} cannot be blank.`})} className={`form-control ${res.className}`}  data-target={res.dataTarget} readOnly={res.specialFeature.includes("readonly")?true:false}/>
                            :
                            <input {...props.register(name)} defaultValue={res.defaultValue} className={`form-control ${res.className}`} data-target={res.dataTarget} readOnly={res.specialFeature.includes("readonly")?true:false}/>
                        }
                    </div>
                </div>
            ):
            ...
            (
                <div key={index} className={res.gridSize}>
                    <div className="form-group">
                        <label className="control-label">{res.title}</label>
                        <input className={`d-none`} {...props.register(name)} defaultValue={res.defaultValue} data-target={res.dataTarget}/>
                        <input className={`form-control ${res.className}`} readOnly={res.specialFeature.includes("readonly")?true:false}/>
                    </div>
                </div>
            ):
            ...
            (
                <div key={index} className={res.gridSize}>
                    <div className="form-group">
                        <label className="control-label">{res.title}</label>
                        {res.specialFeature.includes("required") ? 
                        <input type={"number"} {...props.register(name,{required: `${res.title} cannot be blank.`})} data-target={res.dataTarget} className={`form-control ${res.className}`}/>
                            :   
                        <input type={"number"} {...props.register(name)} className={`form-control ${res.className}`} data-target={res.dataTarget} />
                        }
                    </div>
                </div>
            ):
            ...
            (
                <div key={index} className={res.gridSize}>
                    ...
                    <input type={"text"} className="valid d-none" {...props.register(name)} defaultValue={res.specialFeature.includes("defaultCheck")? 1:0}/>
                    <label htmlFor={res.id} className="control-label ml-1">{res.title}</label>
                    </div>
                </div>
            ...
            (
                <div key={index} className={res.gridSize}>
                    <div className="form-group">
                        <label className="control-label">{res.title}</label>
                        { props.DocumentItem.formName=="ContainerReleaseOrder" || props.DocumentItem.formName=="DeliveryOrder"? res.title=="BR No."? <button type="button" className="BookingLink" style={{"padding": "0px 2px"}}><i class="fa fa-link"></i></button>:"":"" }
                        { props.DocumentItem.formName=="ContainerReleaseOrder" || props.DocumentItem.formName=="DeliveryOrder" || props.DocumentItem.formName=="BookingReservation"? res.title=="QT No."? <button type="button" className="QuotationLink" style={{"padding": "0px 2px"}}><i class="fa fa-link"></i></button>:"":"" }
                        { props.DocumentItem.formName=="DeliveryOrder"? res.title=="BL No."? <button type="button" className="BillOfLadingLink" style={{"padding": "0px 2px"}}><i class="fa fa-link"></i></button>:"":"" }
                        <Controller
                            name={name}
                            control={props.control}
                            ...
                            render={({ field: { onChange, value } }) => (
                                <Select
                                    ...
                                    value={
                                        value
                                        ? res.option?res.option.find((c) => c.value === value) : null
                                        : null
                                    }
                                    onChange={(val) =>{
                ...
                <div key={index} className={res.gridSize}>
                    <div className="form-group">
                        ...
                        <Controller
                            ...
                            render={({ field: { onChange, value } }) => (
                                <Select
                                    {...props.register(name, { required: `Booking Reservation cannot be blank.` })}
                                    isClearable={true}
                                    data-target={res.dataTarget}
                                    id={res.id}
                                    value={
                                    ...
                                    options={res.option}
                                    className={`form-control ${res.className} ${props.errors[props.DocumentItem.formName] ? props.errors[props.DocumentItem.formName][`BookingReservation`] && "has-error-select": ""}`}
                                    classNamePrefix="select"
                                    menuPortalTarget={document.body}
                                    onMenuOpen={val => val == null ? res.onOpenMenu(null) :res.onOpenMenu(val)}
                                    styles={props.verificationStatus ? globalContext.customStylesReadonly : globalContext.customStyles}
                                />
                            )}
                        />               
                    </div>    
                    <p>{props.errors[props.DocumentItem.formName] ? props.errors[props.DocumentItem.formName][`BookingReservation`] && <span style={{ color: "#A94442" }}>{props.errors[props.DocumentItem.formName][`BookingReservation`].message}</span> : ""}</p>
                </div>
            )
            :
            (
