...
export async function manifestCollectAttachment(data,auth) {

  const response = await $.ajax({
    type: "POST",
    url: auth.globalHost + auth.globalPathLink + "manifest/collect-attachment",
    dataType: "json",
    headers: {
      "Authorization": "Basic " + btoa(auth.authInfo.username + ":" + auth.authInfo.access_token)
    },
...
export async function PreviewOR(auth, model, id, type) {
  const response = await axios.get(auth.globalHost + auth.globalPathLink + model + "/preview?id=" + id +"&type=" + type, {
    method:"GET",
    responseType: 'arraybuffer',
    auth: {
      username: auth.authInfo.username,
...
export async function FindRemainingBC(auth,type) {
  var Link;
  if (type == undefined) {
  ...
  const response = await $.ajax({
    type: "GET",
    url: auth.globalHost + auth.globalPathLink + Link+"/get-remaining-booking-confirmation",
    dataType: "json",
    headers: {
      "Authorization": "Basic " + btoa(auth.authInfo.username + ":" + auth.authInfo.access_token)
    },
...
export async function FindBLRemainingBR(auth,type) {
  var Link;
  if (type == undefined) {
    Link = "bill-of-lading";
  }else if(type=="barge"){
  ...
  const response = await $.ajax({
    type: "GET",
    url: auth.globalHost + auth.globalPathLink + Link+"/find-remaining-b-r",
    dataType: "json",
    headers: {
      "Authorization": "Basic " + btoa(auth.authInfo.username + ":" + auth.authInfo.access_token)
    },
...
export async function getChargesWithPortCodeAndEmptyContainerType(id, auth) {
  const response = await $.ajax({
    type: "POST",
    url: auth.globalHost + auth.globalPathLink + "universal/get-charges-with-port-code-and-empty-container-type?VoyageUUID="+id,
    dataType: "json",
    headers: {
      "Authorization": "Basic " + btoa(auth.authInfo.username + ":" + auth.authInfo.access_token)
    },
...
export async function getTransferFromQuotationData(id, auth,type) {
  var Link;
  if (type == undefined) {
    Link = "booking-reservation";
  }else if(type=="barge"){
  ...
  const response = await axios.get(auth.globalHost + auth.globalPathLink +Link+"/transfer-from-quotation-data?id=" + id, {
    auth: {
      username: auth.authInfo.username,
      password: auth.authInfo.access_token,
    },
...
export async function getINVTransferFromBooking(id,CustomerType,BillTo,ChargesID, auth,model,type) {
  var Link;
  if (type == undefined) {
    Link = "sales-invoice";
  }else if(type=="barge"){
  ...
  const response = await axios.get(auth.globalHost + auth.globalPathLink +Link+"/transfer-from-booking-confirmation-data?id=" + id+"&BillTo="+BillTo+"&CustomerType="+CustomerType+"&ChargesID="+stringChargesID, {
    auth: {
      username: auth.authInfo.username,
      password: auth.authInfo.access_token,
    },
...
export async function SaveManifestImportVoyageNo(auth,BLuuids,voyageid) {

  const response = await $.ajax({
    type: "POST",
    url: auth.globalHost + auth.globalPathLink+"manifest-import/save-manifest-import-voyage-no",
    dataType: "json",
    headers: {
      "Authorization": "Basic " + btoa(auth.authInfo.username + ":" + auth.authInfo.access_token)
    },
    data:{
      ManifestImportVoyageNo:voyageid,
      BillOfLadingUUIDs: BLuuids
    },
  });
  return response
}
